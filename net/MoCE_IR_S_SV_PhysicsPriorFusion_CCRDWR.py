"""
MoCE-IR with Three Innovations Fusion
融合三个创新点的完整网络实现
...
- HeteroExpertWithConfidence: 支持物理先验融合的异构专家（深度、光谱、光学参数）

创新点3: 协同竞争路由与动态专家权重重缩放 (Collaborative Competitive Routing with Dynamic Weight Rescaling, CCR-DWR)
- ExpertConfidencePredictor: 专家置信度预测器
- 动态权重重缩放: 根据专家反馈的置信度，动态调整最终的融合权重
...
##########################################################################
## 创新点2+3融合: 带置信度预测的物理先验融合专家

class HeteroExpertWithConfidence(nn.Module):
    """
    融合创新点2和3的专家：
    - 支持物理先验融合（创新点2）
    - 支持置信度预测（创新点3）
    ...
    def __init__(self, dim: int, rank: int, func: nn.Module, depth: int, patch_size: int, kernel_size: int, 
                 expert_type: str = "standard", fusion_type: str = "gated"):
        super(HeteroExpertWithConfidence, self).__init__()
...
##########################################################################
## 创新点3: 带置信度预测的标准专家（不使用物理先验）

class ModExpertWithConfidence(nn.Module):
    """
    带置信度预测的标准专家：在ModExpert基础上添加置信度预测器
    用于不使用物理先验的情况
    """
    def __init__(self, dim: int, rank: int, func: nn.Module, depth: int, patch_size: int, kernel_size: int):
...
########################################################################### 
## Adapter Layer (支持物理先验融合和动态权重重缩放)
class AdapterLayer(nn.Module):
    def __init__(self, 
        ...
        # 创新点3: 使用带置信度预测的融合专家（支持物理先验融合和置信度预测）
        # 如果use_physics_prior=True，使用HeteroExpertWithConfidence；否则使用带置信度的ModExpert
        if use_physics_prior:
            self.experts = nn.ModuleList([
                MySequential(*[HeteroExpertWithConfidence(dim, rank=rank, func=expert_layer, depth=depth, patch_size=patch, 
    ...
    def forward(self, x, freq_emb, shared, raw_physics_priors=None):
        ...
        if self.training:
            ...
            if self.use_physics_prior and encoded_physics_priors is not None:
                ...
                for exp in range(len(self.experts)):
                    output, confidence = self.experts[exp](expert_inputs[exp], expert_shared_intputs[exp])
                    expert_outputs.append(output)
                    expert_confidences.append(confidence)
            
            # 训练模式：简化处理，直接使用gates权重（置信度调整在推理时更有效）
            ...
            if self.use_physics_prior and encoded_physics_priors is not None:
                for expert in selected_experts:
                    output, confidence = expert(x, shared, encoded_physics_priors)
                    expert_outputs.append(output)
                    expert_confidences.append(confidence)
            else:
                for expert in selected_experts:
