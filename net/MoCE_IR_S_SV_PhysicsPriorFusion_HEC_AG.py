...
##########################################################################
## 创新点3: 层次化专家协同与自适应门控 (Hierarchical Expert Collaboration with Adaptive Gating, HEC-AG)
## 核心思想：将专家组织成多个层次，每个层次处理不同尺度的退化，通过跨层次协同和自适应门控实现更精细的修复

class CrossHierarchyCollaboration(nn.Module):
    """
    跨层次协同模块：实现不同层次专家之间的信息交换
    数学原理：通过注意力机制建模层次间的依赖关系，使得粗粒度层次可以指导细粒度层次，反之亦然
    """
    def __init__(self, dim, num_heads=4):
        ...
        # 跨层次注意力：一个层次的特征作为query，另一个层次的特征作为key和value
        self.cross_attn = CrossAttention(dim, num_heads, bias=False)
    ...
    def forward(self, feat_coarse, feat_fine):
        """
        Args:
            feat_coarse: 粗粒度层次特征 (B, C, H, W)
            feat_fine: 细粒度层次特征 (B, C, H, W)
        Returns:
            enhanced_coarse: 增强的粗粒度特征
            enhanced_fine: 增强的细粒度特征
        """
        # 粗粒度 -> 细粒度：粗粒度特征指导细粒度特征
        feat_fine_norm = self.norm1(feat_fine)
        enhanced_fine = self.cross_attn(feat_fine_norm, feat_coarse) + feat_fine
        
        # 细粒度 -> 粗粒度：细粒度特征补充粗粒度特征
        feat_coarse_norm = self.norm2(feat_coarse)
        enhanced_coarse = self.cross_attn(feat_coarse_norm, feat_fine) + feat_coarse
        
        # 门控融合：自适应融合两个层次的特征
        combined = torch.cat([enhanced_coarse, enhanced_fine], dim=1)
        gate = self.gate_conv(combined)
        fused = self.fusion_conv(combined)
        
        # 残差连接
        enhanced_coarse = enhanced_coarse + gate * fused
        enhanced_fine = enhanced_fine + (1 - gate) * fused
        
        return enhanced_coarse, enhanced_fine


class AdaptiveHierarchyGating(nn.Module):
    """
    自适应层次门控：根据输入特征动态调整不同层次的激活权重
    数学原理：通过可学习的门控网络，根据输入特征的复杂度自适应选择激活的层次
    """
    def __init__(self, dim, freq_dim, num_hierarchies=2):
        super().__init__()
        ...
        self.complexity_estimator = nn.Sequential(
            ...
            nn.Linear(dim, dim // 2),
            nn.GELU(),
            nn.Linear(dim // 2, num_hierarchies)
        )
        
        # 频率感知门控
        self.freq_gate = nn.Linear(freq_dim, num_hierarchies, bias=False)
        
        # 物理先验门控（如果使用物理先验）
        self.physics_gate = None
        
    def forward(self, x, freq_emb, physics_prior_feat=None):
        """
        Args:
            x: 输入特征 (B, C, H, W)
            freq_emb: 频率嵌入 (B, freq_dim)
            physics_prior_feat: 物理先验特征 (B, C, H, W) 或 None
        Returns:
            hierarchy_weights: 层次权重 (B, num_hierarchies)
        """
        # 评估特征复杂度
        complexity_logits = self.complexity_estimator(x)  # (B, num_hierarchies)
        
        # 频率感知门控
        freq_logits = self.freq_gate(freq_emb)  # (B, num_hierarchies)
        
        # 融合复杂度和频率信息
        combined_logits = complexity_logits + freq_logits
        
        # 如果使用物理先验，添加物理先验门控
        if physics_prior_feat is not None and self.physics_gate is not None:
            physics_logits = self.physics_gate(physics_prior_feat)
            combined_logits = combined_logits + physics_logits
        
        # Softmax归一化得到层次权重
        hierarchy_weights = F.softmax(combined_logits, dim=-1)  # (B, num_hierarchies)
        
        return hierarchy_weights


class HierarchicalExpertGroup(nn.Module):
    """
    层次化专家组：将专家组织成多个层次，每个层次处理不同尺度的退化
    数学原理：通过层次化组织，可以更好地建模多尺度退化模式，实现粗到细的渐进式修复
    """
    def __init__(self, dim, rank, num_experts_per_hierarchy, num_hierarchies=2, 
                 expert_layer=FFTAttention, depth_type="lin", rank_type="constant",
                 stage_depth=1, use_physics_prior=False, expert_types=None):
        super().__init__()
        self.num_hierarchies = num_hierarchies
        self.num_experts_per_hierarchy = num_experts_per_hierarchy
        self.use_physics_prior = use_physics_prior
        
        # 为每个层次创建专家组
        self.hierarchies = nn.ModuleList()
        for h_idx in range(num_hierarchies):
            # 不同层次使用不同的patch_size和kernel_size，实现多尺度处理
            patch_sizes = [2**(h_idx*2 + i+2) for i in range(num_experts_per_hierarchy)]
            kernel_sizes = [3+(h_idx*2 + 2*i) for i in range(num_experts_per_hierarchy)]
            
            if depth_type == "lin":
                depths = [stage_depth + h_idx + i for i in range(num_experts_per_hierarchy)]
            elif depth_type == "constant":
                depths = [stage_depth for _ in range(num_experts_per_hierarchy)]
            else:
                depths = [stage_depth + i for i in range(num_experts_per_hierarchy)]
            
            if rank_type == "constant":
                ranks = [rank for _ in range(num_experts_per_hierarchy)]
            elif rank_type == "spread":
                ranks = [dim//(2**(h_idx + i)) for i in range(num_experts_per_hierarchy)][::-1]
            else:
                ranks = [rank + i for i in range(num_experts_per_hierarchy)]
            
            # 创建该层次的专家
            if use_physics_prior and expert_types is not None:
                hierarchy_experts = nn.ModuleList([
                    MySequential(*[HeteroExpert(dim, rank=r, func=expert_layer, depth=d, 
                                              patch_size=p, kernel_size=k, 
                                              expert_type=expert_types[i] if i < len(expert_types) else "standard")])
                    for i, (d, r, p, k) in enumerate(zip(depths, ranks, patch_sizes, kernel_sizes))
                ])
            else:
                hierarchy_experts = nn.ModuleList([
                    MySequential(*[ModExpert(dim, rank=r, func=expert_layer, depth=d, 
                                           patch_size=p, kernel_size=k)])
                    for d, r, p, k in zip(depths, ranks, patch_sizes, kernel_sizes)
                ])
            
            self.hierarchies.append(hierarchy_experts)
        
        # 跨层次协同模块
        self.cross_collaboration = nn.ModuleList([
            CrossHierarchyCollaboration(dim, num_heads=4)
            for _ in range(num_hierarchies - 1)
        ])
        
    def forward(self, x, shared, hierarchy_weights, encoded_physics_priors=None):
        """
        Args:
            x: 输入特征 (B, C, H, W)
            shared: 共享特征 (B, C, H, W)
            hierarchy_weights: 层次权重 (B, num_hierarchies)
            encoded_physics_priors: 编码的物理先验特征
        Returns:
            output: 层次化专家输出 (B, C, H, W)
        """
        B, C, H, W = x.shape
        hierarchy_outputs = []
        
        # 每个层次独立处理：使用该层次的所有专家，然后加权聚合
        for h_idx, hierarchy_experts in enumerate(self.hierarchies):
            if len(hierarchy_experts) > 0:
                # 使用该层次的所有专家处理输入
                expert_outputs = []
                for expert in hierarchy_experts:
                    if self.use_physics_prior and encoded_physics_priors is not None:
                        expert_output = expert(x, shared, encoded_physics_priors)
                    else:
                        expert_output = expert(x, shared)
                    expert_outputs.append(expert_output)
                
                # 简单平均聚合该层次的专家输出（可以改为加权聚合）
                hierarchy_output = torch.stack(expert_outputs, dim=0).mean(dim=0)
                hierarchy_outputs.append(hierarchy_output)
            else:
                hierarchy_outputs.append(x)
        
        # 跨层次协同：相邻层次之间进行信息交换
        for i in range(len(hierarchy_outputs) - 1):
            hierarchy_outputs[i], hierarchy_outputs[i+1] = self.cross_collaboration[i](
                hierarchy_outputs[i], hierarchy_outputs[i+1]
            )
        
        # 根据层次权重加权聚合
        hierarchy_weights_expanded = hierarchy_weights.unsqueeze(2).unsqueeze(3).unsqueeze(4)  # (B, num_hierarchies, 1, 1, 1)
        hierarchy_outputs_tensor = torch.stack(hierarchy_outputs, dim=1)  # (B, num_hierarchies, C, H, W)
        
        output = torch.sum(hierarchy_weights_expanded * hierarchy_outputs_tensor, dim=1)  # (B, C, H, W)
        
        return output


########################################################################### 
## Adapter Layer (支持物理先验融合和层次化专家协同)
class AdapterLayer(nn.Module):
    def __init__(self, 
                 ...
                 use_hierarchical: bool=True, num_hierarchies: int=2):
        super().__init__()            
        ...
        self.num_hierarchies = num_hierarchies if use_hierarchical else 1
        ...
        if use_hierarchical:
            # 计算每个层次的专家数量
            num_experts_per_hierarchy = num_experts // num_hierarchies
            if num_experts_per_hierarchy == 0:
                num_experts_per_hierarchy = 1
            
            # 创建层次化专家组
            self.hierarchical_experts = HierarchicalExpertGroup(
                dim=dim, rank=rank, 
                num_experts_per_hierarchy=num_experts_per_hierarchy,
            ...
            # 自适应层次门控
            self.adaptive_gating = AdaptiveHierarchyGating(
                dim=dim, freq_dim=freq_dim, num_hierarchies=num_hierarchies
            )
            
            # 为了兼容性，仍然保留传统的路由（用于辅助损失）
            ...
            if use_physics_prior:
                self.experts = nn.ModuleList([
                    MySequential(*[HeteroExpert(dim, rank=r, func=expert_layer, depth=d, 
                                              patch_size=p, kernel_size=k, expert_type=expert_types[i] if i < len(expert_types) else "standard")])
                    for i, (d, r, p, k) in enumerate(zip(depths, ranks, patch_sizes, kernel_sizes))
                ])
            else:
                self.experts = nn.ModuleList([
        ...
        if self.use_hierarchical:
            ...
            if self.use_physics_prior and raw_physics_priors is not None:
                ...
                if "depth_map" in raw_physics_priors and raw_physics_priors["depth_map"] is not None:
                    physics_prior_feat = raw_physics_priors["depth_map"]
            
            # 自适应层次门控：根据输入特征动态调整层次权重
            hierarchy_weights = self.adaptive_gating(x, freq_emb, physics_prior_feat)
            
            # 编码物理先验（如果需要）
            encoded_physics_priors = None
            if self.use_physics_prior and raw_physics_priors is not None:
                # 这里简化处理，实际应该使用RoutingFunction中的编码器
                encoded_physics_priors = {}
                if "depth_map" in raw_physics_priors and raw_physics_priors["depth_map"] is not None:
                    encoded_physics_priors["depth_feat"] = raw_physics_priors["depth_map"]
                if "spectral_data" in raw_physics_priors and raw_physics_priors["spectral_data"] is not None:
                    encoded_physics_priors["spectral_feat"] = raw_physics_priors["spectral_data"]
                if "optical_params" in raw_physics_priors and raw_physics_priors["optical_params"] is not None:
                    encoded_physics_priors["optical_param_feat"] = raw_physics_priors["optical_params"]
            
            # 层次化专家处理
            out = self.hierarchical_experts(x, shared, hierarchy_weights, encoded_physics_priors)
            
            # 为了计算辅助损失，仍然运行传统路由（但不使用其输出）
            gates, top_k_indices, top_k_values, aux_loss, _ = self.routing(x, freq_emb, raw_physics_priors)
            self.loss = aux_loss
        else:
            # 传统非层次化架构
...
class DecoderBlock(nn.Module):
    def __init__(self, dim, num_heads, ffn_expansion_factor, bias, LayerNorm_type, expert_layer, complexity_scale=None,
        ...
        self.adapter = AdapterLayer(
            ...
            use_hierarchical=use_hierarchical, num_hierarchies=num_hierarchies
        )
...
def build_model(opt):
    ...
    use_hierarchical = getattr(opt, 'use_hierarchical', True)
    num_hierarchies = getattr(opt, 'num_hierarchies', 2)
