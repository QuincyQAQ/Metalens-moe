...
class SpectralConv2d(nn.Module):
    """
    傅里叶神经算子的谱卷积层
    在傅里叶空间中进行线性变换，学习物理算子的逆算子
    
    注意：使用实数参数存储复数权重（实部和虚部分开），以兼容AMP混合精度训练
    """
    def __init__(self, in_channels, out_channels, modes1, modes2):
        ...
        # 使用实数参数存储复数权重（实部和虚部分开），以兼容AMP
        # weights1_real 和 weights1_imag 组合成 weights1 = weights1_real + 1j * weights1_imag
        self.weights1_real = nn.Parameter(self.scale * torch.randn(in_channels, out_channels, self.modes1, self.modes2))
        self.weights1_imag = nn.Parameter(self.scale * torch.randn(in_channels, out_channels, self.modes1, self.modes2))
        self.weights2_real = nn.Parameter(self.scale * torch.randn(in_channels, out_channels, self.modes1, self.modes2))
        self.weights2_imag = nn.Parameter(self.scale * torch.randn(in_channels, out_channels, self.modes1, self.modes2))

    def _get_complex_weights(self, real_param, imag_param, complex_dtype):
        """将实数参数组合成复数权重"""
        # 将实数参数转换为复数
        weights = torch.complex(real_param, imag_param)
        # 转换为目标复数类型
        if complex_dtype == torch.complex32:  # ComplexHalf
            weights = weights.to(torch.complex64)  # 先转换为ComplexFloat进行计算
        else:
    ...
    def compl_mul2d(self, input, weights):
        ...
        # torch.einsum doesn't support ComplexHalf, so we need to convert to ComplexFloat for computation
        input_dtype = input.dtype
        if input_dtype == torch.complex32:  # ComplexHalf
            input = input.to(torch.complex64)  # Convert to ComplexFloat
            weights = weights.to(torch.complex64)
            result = torch.einsum("bixy,ioxy->boxy", input, weights)
            return result.to(input_dtype)  # Convert back to original dtype
        else:
    ...
    def forward(self, x):
        ...
        # Determine complex dtype based on input
        # If input is half precision, FFT returns ComplexHalf, otherwise ComplexFloat
        complex_dtype = x_ft.dtype
        
        # 动态调整 modes 以适应输入尺寸
        # rfft2 返回的最后一个维度是 W//2+1，需要确保 modes 不超过这个值
        actual_modes1 = min(self.modes1, H)
        actual_modes2 = min(self.modes2, W // 2 + 1)
        
        # 将实数参数组合成复数权重，并切片到实际使用的 modes
        weights1 = self._get_complex_weights(
            self.weights1_real[:, :, :actual_modes1, :actual_modes2],
            self.weights1_imag[:, :, :actual_modes1, :actual_modes2],
            complex_dtype
        )
        weights2 = self._get_complex_weights(
            self.weights2_real[:, :, :actual_modes1, :actual_modes2],
            self.weights2_imag[:, :, :actual_modes1, :actual_modes2],
            complex_dtype
        )
        
        # Multiply relevant Fourier modes
        out_ft = torch.zeros(B, self.out_channels, H, W // 2 + 1, dtype=complex_dtype, device=x.device)
        
        # Handle low frequency modes (前 modes1 行)
        if actual_modes1 > 0 and actual_modes2 > 0:
            out_ft[:, :, :actual_modes1, :actual_modes2] = \
                self.compl_mul2d(x_ft[:, :, :actual_modes1, :actual_modes2], weights1)
        
        # Handle high frequency modes (后 modes1 行)
        if actual_modes1 > 0 and actual_modes2 > 0 and H > actual_modes1:
            out_ft[:, :, -actual_modes1:, :actual_modes2] = \
                self.compl_mul2d(x_ft[:, :, -actual_modes1:, :actual_modes2], weights2)
...
class FNOLayer(nn.Module):
    ...
    傅里叶神经算子层：结合谱卷积和空间卷积
    """
    def __init__(self, dim, modes1=12, modes2=12):
...
## FNO Expert (替代ModExpert/HeteroExpert)
class FNOExpert(nn.Module):
    """
    创新点3: 神经算子专家
    使用FNO直接学习物理退化算子的逆算子，而非函数逼近
    """
    def __init__(self, dim: int, rank: int, depth: int, patch_size: int, kernel_size: int,
                 expert_type: str = "standard", fusion_type: str = "gated", modes1: int = 12, modes2: int = 12):
        super(FNOExpert, self).__init__()
        ...
        # FNO主体：使用FNO层替代FFTAttention
        self.fno_layers = nn.ModuleList([
            FNOLayer(rank, modes1=modes1, modes2=modes2) for _ in range(depth)
        ])
    ...
    def process(self, x, shared, physics_priors=None):
        ...
        for fno_layer in self.fno_layers:
            x = fno_layer(x)
        x = x * F.silu(self.proj[1](shared))
        x = self.proj[2](x)
        return x + shortcut

    def feat_extract(self, feats, shared, physics_priors=None):
        # FNO层已经在process中堆叠了depth次，这里只需要一次process调用
...
class AdapterLayer(nn.Module):
    def __init__(self, 
                 ...
                 use_physics_prior: bool=False, expert_types: List[str]=None, use_fno: bool=True):
        super().__init__()            
        ...
        self.use_fno = use_fno  # 创新点3: 是否使用FNO专家
        ...
        # 创新点3: 使用FNOExpert（神经算子专家）替代传统专家
        if use_fno:
            # FNO专家：直接学习物理逆算子
            self.experts = nn.ModuleList([
                MySequential(*[FNOExpert(dim, rank=rank, depth=depth, patch_size=patch, 
                                        kernel_size=kernel, expert_type=expert_type if use_physics_prior else "standard",
                                        fusion_type="gated", modes1=12, modes2=12)])
                for idx, (depth, rank, patch, kernel, expert_type) in enumerate(zip(depths, ranks, patch_sizes, kernel_sizes, expert_types))
            ])
        elif use_physics_prior:
...
##########################################################################
## 创新点2+3: 自适应物理约束路由 + 自适应逆问题路由 (AIPR)
## AIPR融合SVFE频率特征和PhysicsPriorFusion物理先验，实现逆问题驱动的智能路由
...
class DecoderBlock(nn.Module):
    def __init__(self, dim, num_heads, ffn_expansion_factor, bias, LayerNorm_type, expert_layer, complexity_scale=None,
                 ...
                 with_complexity: bool=False, use_physics_prior: bool=False, expert_types: List[str]=None, use_fno: bool=True):
        super().__init__()
        ...
        self.adapter = AdapterLayer(
            ...
            use_physics_prior=use_physics_prior, expert_types=expert_types, use_fno=use_fno
        )
        ...
        for i in range(levels-1):
            self.dec.append(nn.ModuleList([
                ...
                DecoderResidualGroup(
                    ...
                    use_physics_prior=use_physics_prior, expert_types=expert_types_list[i] if use_physics_prior else None, use_fno=use_fno),
                ])
            )
...
def build_model(opt):
    ...
    use_fno = getattr(opt, 'use_fno', True)  # 创新点3: 默认使用FNO专家
