...
##########################################################################
## Wavelet Decomposition and Reconstruction (纯PyTorch实现)
class WaveletDecomp(nn.Module):
    """小波分解模块，将输入分解为LL, LH, HL, HH四个频段（纯PyTorch实现）"""
    def __init__(self):
        super(WaveletDecomp, self).__init__()
        
        # Haar小波滤波器（低通和高通）
        # 低通滤波器: [1/sqrt(2), 1/sqrt(2)]
        # 高通滤波器: [1/sqrt(2), -1/sqrt(2)]
        sqrt2 = math.sqrt(2.0)
        
        # 2D Haar小波分解滤波器
        # LL: 低通-低通
        ll_kernel = torch.tensor([[1.0, 1.0], [1.0, 1.0]], dtype=torch.float32) / 2.0
        # LH: 低通-高通 (水平高频)
        lh_kernel = torch.tensor([[1.0, -1.0], [1.0, -1.0]], dtype=torch.float32) / 2.0
        # HL: 高通-低通 (垂直高频)
        hl_kernel = torch.tensor([[1.0, 1.0], [-1.0, -1.0]], dtype=torch.float32) / 2.0
        # HH: 高通-高通 (对角高频)
        hh_kernel = torch.tensor([[1.0, -1.0], [-1.0, 1.0]], dtype=torch.float32) / 2.0
        
        self.register_buffer('ll_kernel', ll_kernel.unsqueeze(0).unsqueeze(0))
        self.register_buffer('lh_kernel', lh_kernel.unsqueeze(0).unsqueeze(0))
        self.register_buffer('hl_kernel', hl_kernel.unsqueeze(0).unsqueeze(0))
        self.register_buffer('hh_kernel', hh_kernel.unsqueeze(0).unsqueeze(0))
    
    def forward(self, x):
        """
        x: [B, C, H, W]
        返回: cA (LL), cH (LH), cV (HL), cD (HH)
        每个都是 [B, C, H//2, W//2]
        """
        b, c, h, w = x.shape
        ...
        for i in range(c):
            ...
            # 使用卷积实现小波分解（stride=2进行下采样）
            cA = F.conv2d(x_ch, self.ll_kernel, stride=2, padding=0)  # [B, 1, H//2, W//2]
            cH = F.conv2d(x_ch, self.lh_kernel, stride=2, padding=0)
            cV = F.conv2d(x_ch, self.hl_kernel, stride=2, padding=0)
            cD = F.conv2d(x_ch, self.hh_kernel, stride=2, padding=0)
        ...
        # 拼接所有通道
        cA = torch.cat(cA_list, dim=1)  # [B, C, H//2, W//2]
        cH = torch.cat(cH_list, dim=1)
        cV = torch.cat(cV_list, dim=1)
        cD = torch.cat(cD_list, dim=1)
        
        return cA, cH, cV, cD


class WaveletRecon(nn.Module):
    """小波重构模块，将四个频段重构为原始图像（纯PyTorch实现）"""
    def __init__(self):
        super(WaveletRecon, self).__init__()
        
        # Haar小波重构滤波器（逆变换）
        # 低通重构: [1/sqrt(2), 1/sqrt(2)]
        # 高通重构: [-1/sqrt(2), 1/sqrt(2)]
        sqrt2 = math.sqrt(2.0)
    ...
    def forward(self, cA, cH, cV, cD, target_size=None):
        """
        cA, cH, cV, cD: [B, C, H//2, W//2]
        target_size: (H, W) 目标输出尺寸，如果为None则自动计算
        返回: [B, C, H, W]
        """
        b, c, h, w = cA.shape
        
        # 计算目标输出尺寸（转置卷积stride=2会将尺寸翻倍）
        ...
        for i in range(c):
            cA_ch = cA[:, i:i+1, :, :]  # [B, 1, H//2, W//2]
            cH_ch = cH[:, i:i+1, :, :]
            cV_ch = cV[:, i:i+1, :, :]
            cD_ch = cD[:, i:i+1, :, :]
            
            # 使用转置卷积实现小波重构（stride=2进行上采样）
            # 注意：转置卷积的输出尺寸 = (input_size - 1) * stride + kernel_size
            # 对于2x2 kernel, stride=2: output = (h-1)*2 + 2 = 2h
            ll_up = F.conv_transpose2d(cA_ch, self.ll_kernel, stride=2, padding=0, output_padding=0)  # [B, 1, ~2h, ~2w]
            lh_up = F.conv_transpose2d(cH_ch, self.lh_kernel, stride=2, padding=0, output_padding=0)
            hl_up = F.conv_transpose2d(cV_ch, self.hl_kernel, stride=2, padding=0, output_padding=0)
            hh_up = F.conv_transpose2d(cD_ch, self.hh_kernel, stride=2, padding=0, output_padding=0)
            
            # 重构 = 四个频段相加
            recon = ll_up + lh_up + hl_up + hh_up  # [B, 1, ~2h, ~2w]
            
            # 调整到目标尺寸
            if recon.shape[2] != target_h or recon.shape[3] != target_w:
                recon = F.interpolate(recon, size=(target_h, target_w), mode='bilinear', align_corners=False)
            
            recon_list.append(recon)
        
        # 拼接所有通道
        recon = torch.cat(recon_list, dim=1)  # [B, C, target_h, target_w]
...
class MoCEIR(nn.Module):
    def __init__(self,
        ...
        # -- Wavelet Decomposition (Encoder输入端)
        self.wavelet_decomp = WaveletDecomp()
        
        # -- 为4个小波频段创建4个专家处理模块
        # LL (低频), LH (水平高频), HL (垂直高频), HH (对角高频)
        assert num_experts == 4, "Wavelet network requires num_experts=4 for LL, LH, HL, HH"
        
        # 每个频段的特征投影
        self.wavelet_projs = nn.ModuleList([
            nn.Conv2d(inp_channels, dim, kernel_size=3, padding=1, bias=False)  # LL
            for _ in range(4)  # LL, LH, HL, HH
        ])
        
        # -- Patch Embedding (用于融合后的特征)
        self.patch_embed = OverlapPatchEmbed(in_c=dim, embed_dim=dim, bias=False)
        ...
        # -- Wavelet Reconstruction (Decoder端)
        self.wavelet_recon = WaveletRecon()
        
        # 小波重构前的特征投影
        self.wavelet_recon_projs = nn.ModuleList([
            nn.Conv2d(dim, inp_channels, kernel_size=3, padding=1, bias=False)  # 投影回原始通道数
            for _ in range(4)  # LL, LH, HL, HH
        ])
        
        self.output = nn.Conv2d(out_channels, out_channels, kernel_size=3, stride=1, padding=1, bias=bias)
    ...
    def forward(self, x, labels=None):
        ...
        # ========== Encoder端：小波分解 ==========
        # 1. 小波分解为4个频段
        cA, cH, cV, cD = self.wavelet_decomp(x)  # 每个都是 [B, C, H//2, W//2]
        h_half, w_half = cA.shape[2], cA.shape[3]
        
        # 2. 每个频段通过对应的投影和专家处理
        # LL (低频) -> 专家0
        ll_feat = self.wavelet_projs[0](cA)  # [B, dim, H//2, W//2]
        # LH (水平高频) -> 专家1
        lh_feat = self.wavelet_projs[1](cH)  # [B, dim, H//2, W//2]
        # HL (垂直高频) -> 专家2
        hl_feat = self.wavelet_projs[2](cV)  # [B, dim, H//2, W//2]
        # HH (对角高频) -> 专家3
        hh_feat = self.wavelet_projs[3](cD)  # [B, dim, H//2, W//2]
        
        # 3. 融合4个频段的特征（简单相加或拼接）
        feats = ll_feat + lh_feat + hl_feat + hh_feat  # [B, dim, H//2, W//2]
        
        # 4. 通过patch embedding进一步处理
        feats = self.patch_embed(feats)
        ...
        # ========== Decoder端：小波重构 ==========
        # 1. 将特征投影为4个频段
        ll_out = self.wavelet_recon_projs[0](feats)  # [B, C, H_feat, W_feat]
        lh_out = self.wavelet_recon_projs[1](feats)
        hl_out = self.wavelet_recon_projs[2](feats)
        hh_out = self.wavelet_recon_projs[3](feats)
        
        # 2. 调整到小波频段尺寸（确保与分解时的尺寸匹配）
        # feats的尺寸可能不是h_half x w_half，需要调整
        ll_out = F.interpolate(ll_out, size=(h_half, w_half), mode='bilinear', align_corners=False)
        lh_out = F.interpolate(lh_out, size=(h_half, w_half), mode='bilinear', align_corners=False)
        hl_out = F.interpolate(hl_out, size=(h_half, w_half), mode='bilinear', align_corners=False)
        hh_out = F.interpolate(hh_out, size=(h_half, w_half), mode='bilinear', align_corners=False)
        
        # 3. 小波重构（指定目标尺寸为原始输入尺寸）
        recon = self.wavelet_recon(ll_out, lh_out, hl_out, hh_out, target_size=(h_orig, w_orig))  # [B, C, H_orig, W_orig]
        
        # 5. 最终输出（残差连接）
        x = self.output(recon) + x
...
def build_model(opt):
    ...
    # Wavelet网络需要4个专家对应LL, LH, HL, HH
    num_experts = 4  # 固定为4，对应小波分解的4个频段
