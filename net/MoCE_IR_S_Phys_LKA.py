...
##########################################################################
## 核心创新模块：物理参数化动态 LKA (Phys-LKA)
## Physics-Parameterized Dynamic Large-Kernel Attention
## 
## 学术叙事逻辑：
## 1. 物理背景: Metalens 的 PSF 随物理条件（如波长、入射角）动态变化，需要 LKA 具备更深度的物理适应性。
## 2. 创新点: 利用 phys_emb 动态生成 LKA 的调制权重，实现真正的“物理驱动算子”。
## 3. 优势: 将物理先验从简单的门控提升为 LKA 内部行为的“动态控制器”，使 LKA 能够根据物理上下文自适应地调整其空间滤波特性。
##########################################################################
...
class Phys_LKA_Expert(nn.Module):
    """
    [学术定义]: Physics-Parameterized Dynamic LKA Expert (PD-LKA-E) / 物理参数化动态 LKA 专家
    
    核心逻辑: 融合 LKA 的全局空间感知与物理先验驱动的动态调制，直接影响 LKA 的内部行为。
    创新点: 实现了“物理参数化动态 LKA”，使 LKA 能够根据输入的物理上下文动态调整其空间滤波特性。
    """
    ...
    def __init__(self, dim: int, phys_dim: int, expansion_ratio: float = 2.0):
        ...
        # 3. [核心创新]: 物理参数化 LKA 调制器 (Physics-Parameterized LKA Modulator)
        # 将物理嵌入映射为 LKA 内部的动态调制参数，例如用于调制 LKA 的 conv1 的权重或偏置。
        # 这里我们选择调制 LKA 的输出，作为一种动态的通道注意力。
        self.phys_lka_modulator = nn.Sequential(
            nn.Linear(phys_dim, self.hidden_dim),
            nn.ReLU(inplace=True),
            nn.Linear(self.hidden_dim, self.hidden_dim),
            nn.Sigmoid(), # 生成 0-1 范围的调制系数
        )
    ...
    def forward(self, x: torch.Tensor, phys_emb: torch.Tensor) -> torch.Tensor:
        ...
        x_main_lka_out = self.lka(x_main)

        # 3. 物理参数化 LKA 调制：动态调整 LKA 的输出
        lka_mod_factor = self.phys_lka_modulator(phys_emb).view(b, -1, 1, 1)
        x_main = x_main_lka_out * lka_mod_factor # 物理先验直接调制 LKA 的空间响应

        # 4. 物理域调制：生成物理引导的动态门控 (Physics-Guided Dynamic Gating)
        p_gate = self.physics_gate_gen(phys_emb).view(b, -1, 1, 1)
        gate = gate * p_gate

        # 5. 协同门控融合 (Co-Gating Fusion)
...
class Phys_LKA_Router(nn.Module):
    """
...
class Phys_LKA_AdapterLayer(nn.Module):
    """
    [学术定义]: Physics-Aware Dynamically-Modulated Mixture-of-Experts (PA-DM-MoE) Layer
    
    功能: 封装了物理参数化动态 LKA 专家与路由机制，作为 MoCE-IR 架构的核心计算单元。
    """
    ...
    def __init__(self, dim: int, phys_dim: int, num_experts: int = 4, k: int = 1):
        ...
        self.experts = nn.ModuleList(
            [Phys_LKA_Expert(dim, phys_dim) for _ in range(num_experts)]
        )
        # 物理引导路由器
        self.router = Phys_LKA_Router(dim, phys_dim, num_experts, k)
...
##########################################################################
## 简化版 MoCE-IR 主干：UNet 结构 + PA-DM-MoE 适配层
##########################################################################
...
class MoCEIR_Phys_LKA(nn.Module):
    """
    [学术定义]: Physics-Informed Dynamically-Modulated Mixture-of-Conditional-Experts for Image Restoration
    
    架构描述: 
    1. 采用对称的 Encoder-Decoder 结构作为主干。
    2. 在 Decoder 阶段引入 PA-DM-MoE 层，实现对多尺度物理退化的自适应修复。
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # 在深层特征空间引入 PA-DM-MoE，处理复杂的全局退化
        self.phys_lka_adapter2 = Phys_LKA_AdapterLayer(dim * 4, phys_dim, num_experts=num_experts, k=topk)

        self.up1 = Upsample(dim * 4, dim * 2)
        self.dec1 = ConvBlock(dim * 3, dim * 2)
        # 在浅层特征空间引入 PA-DM-MoE，修复精细的空间变异细节
        self.phys_lka_adapter1 = Phys_LKA_AdapterLayer(dim * 2, phys_dim, num_experts=num_experts, k=topk)
    ...
    def forward(
        ...
        d_dec2 = self.phys_lka_adapter2(d_dec2, global_phys_emb)
        ...
        d_dec1 = self.phys_lka_adapter1(d_dec1, global_phys_emb)
...
if __name__ == "__main__":
    ...
    model = MoCEIR_Phys_LKA(inp_channels=in_channels, out_channels=in_channels, dim=feature_dim, phys_dim=physical_embedding_dim, num_experts=num_experts)
