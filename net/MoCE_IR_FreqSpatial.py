...
##########################################################################
## Self-Attention in Fourier Domain with Frequency Masking
class FFTAttentionFreqMask(nn.Module):
    """
    FFT Attention with frequency band masking.
    Expert 1: DC-0.1π (low freq)
    Expert 2: 0.1π-0.3π (mid-low freq)
    Expert 3: 0.3π-0.6π (mid-high freq)
    Expert 4: 0.6π-0.9π (high freq)
    """
    def __init__(self, dim: int, freq_range: Tuple[float, float] = (0.0, 1.0), **kwargs):
        super(FFTAttentionFreqMask, self).__init__()

        self.patch_size = kwargs["patch_size"]
        self.freq_range = freq_range  # (low, high) in normalized frequency [0, 1]
    ...
    def create_freq_mask(self, h, w, device):
        """Create frequency mask for the specified frequency range.
        For rfft2, the output shape is [H, W] where H=patch_size, W=patch_size//2+1.
        This mask is applied at the patch level.
        """
        # h = patch_size, w = patch_size//2+1 (for rfft2 output)
        # Create frequency coordinates
        u = torch.arange(h, device=device).float()
        v = torch.arange(w, device=device).float()
        
        # Normalize to frequency domain [0, 1]
        # For u direction: map to [-0.5, 0.5] then take absolute
        u_centered = (u - h // 2) / h
        u_norm = torch.abs(u_centered)
        
        # For v direction: rfft2 only has positive frequencies [0, 0.5]
        # Map to [0, 1] by dividing by 0.5
        v_norm = v / (w - 1) if w > 1 else v  # Normalize to [0, 1]
        
        # Create meshgrid
        U, V = torch.meshgrid(u_norm, v_norm, indexing='ij')
        
        # Compute frequency magnitude (normalized to [0, 1])
        freq_mag = torch.sqrt(U**2 + V**2)
        freq_mag = freq_mag / (freq_mag.max() + 1e-8)  # Normalize to [0, 1]
        
        # Create mask for the frequency range
        low, high = self.freq_range
        mask = (freq_mag >= low) & (freq_mag < high)
        mask = mask.float()
    ...
    def forward(self, x):
        ...
        # q_fft shape: [B, C, H, W, patch_h, patch_w//2+1] (complex)
        # We need to apply mask to the last two spatial dimensions (patch dimensions)
        patch_h, patch_w_rfft = q_fft.shape[-2], q_fft.shape[-1]
        freq_mask = self.create_freq_mask(patch_h, patch_w_rfft, q_fft.device)
        # Expand mask to match q_fft shape: [1, 1, 1, 1, patch_h, patch_w//2+1]
        while freq_mask.dim() < q_fft.dim():
            freq_mask = freq_mask.unsqueeze(0)
        q_fft = q_fft * freq_mask
        k_fft = k_fft * freq_mask
...
class ModExpert(nn.Module):
    def __init__(self, dim: int, rank: int, func: nn.Module, depth: int, patch_size: int, kernel_size:int, freq_range: Tuple[float, float] = None):
        ...
        # For frequency experts, pass freq_range; for spatial expert, use regular Attention
        if freq_range is not None:
            self.body = func(rank, freq_range=freq_range, kernel_size=kernel_size, patch_size=patch_size)
        else:
...
########################################################################### 
## Adapter Layer with 5 Experts (4 Frequency + 1 Spatial)
class AdapterLayerFreqSpatial(nn.Module):
    def __init__(self, 
                 dim: int, rank: int, num_experts: int = 5, top_k: int=2, expert_layer: nn.Module=FFTAttentionFreqMask, stage_depth: int=1,
        ...
        assert num_experts == 5, "This adapter requires exactly 5 experts (4 frequency + 1 spatial)"

        # Frequency ranges for 4 frequency experts
        # Expert 1: DC-0.1π (global low freq - motion blur, registration)
        # Expert 2: 0.1π-0.3π (mid-low freq - local blur, registration errors)
        # Expert 3: 0.3π-0.6π (mid-high freq - chromatic aberration, slight noise)
        # Expert 4: 0.6π-0.9π (high freq - sharp edges, strong noise)
        freq_ranges = [(0.0, 0.1), (0.1, 0.3), (0.3, 0.6), (0.6, 0.9)]
        
        patch_sizes = [2**(i+2) for i in range(4)] + [8]  # Last one for spatial expert
        kernel_sizes = [3+(2*i) for i in range(4)] + [7]  # Last one for spatial expert
        
        if depth_type == "lin":
            depths = [stage_depth+i for i in range(4)] + [stage_depth+3]
        elif depth_type == "double":
            depths = [stage_depth+(2*i) for i in range(4)] + [stage_depth+6]
        elif depth_type == "exp":
            depths = [2**(i) for i in range(4)] + [8]
        elif depth_type == "fact":
            depths = [math.factorial(i+1) for i in range(4)] + [math.factorial(5)]
        elif isinstance(depth_type, int):
            depths = [depth_type for _ in range(5)]
        elif depth_type == "constant":
            depths = [stage_depth for i in range(5)]
        else:
            raise(NotImplementedError)
        
        if rank_type == "constant":
            ranks = [rank for _ in range(5)]
        elif rank_type == "lin":
            ranks = [rank+i for i in range(5)]
        elif rank_type == "double":
            ranks = [rank+(2*i) for i in range(5)]
        elif rank_type == "exp":
            ranks = [rank**(i+1) for i in range(5)]
        elif rank_type == "fact":
            ranks = [math.factorial(rank+i) for i in range(5)]
        elif rank_type == "spread":
            ranks = [dim//(2**i) for i in range(5)][::-1]
        else:
            raise(NotImplementedError)
        
        # Create 4 frequency experts + 1 spatial expert
        self.experts = nn.ModuleList()
        
        # Frequency experts (0-3)
        for idx in range(4):
            self.experts.append(
                MySequential(*[ModExpert(
                    dim, rank=ranks[idx], func=expert_layer, depth=depths[idx], 
                    patch_size=patch_sizes[idx], kernel_size=kernel_sizes[idx],
                    freq_range=freq_ranges[idx]
                )])
            )
        
        # Spatial expert (4) - uses Attention instead of FFTAttention
        # Create a wrapper function for spatial attention
        def create_spatial_attention(dim, **kwargs):
            return Attention(dim, num_heads=1, bias=False)
        
        self.experts.append(
            MySequential(*[ModExpert(
                dim, rank=ranks[4], func=create_spatial_attention, 
                depth=depths[4], patch_size=patch_sizes[4], kernel_size=kernel_sizes[4],
                freq_range=None
            )])
        )
        ...
        self.routing = RoutingFunctionFreqSpatial(
            dim, freq_dim, 
...
class RoutingFunctionFreqSpatial(nn.Module):
    """
    Dual-path routing: frequency path for experts 0-3, spatial path for expert 4
    """
    def __init__(self, dim, freq_dim, num_experts, k, complexity, use_complexity_bias: bool = True, complexity_scale: str="max"):
        super(RoutingFunctionFreqSpatial, self).__init__()
        
        assert num_experts == 5, "This routing requires exactly 5 experts"
        
        # Frequency gating for experts 0-3
        self.freq_gate = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            Rearrange('b c 1 1 -> b c'),
            nn.Linear(dim, 4, bias=False)  # 4 frequency experts
        )
        self.freq_gate_emb = nn.Linear(freq_dim, 4, bias=False)
        
        # Spatial gating for expert 4
        self.spatial_gate = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            Rearrange('b c 1 1 -> b c'),
            nn.Linear(dim, 1, bias=False)  # 1 spatial expert
        )
        self.spatial_gate_emb = nn.Linear(freq_dim, 1, bias=False)
    ...
    def forward(self, x, freq_emb):
        # Frequency path: experts 0-3
        freq_logits = self.freq_gate(x) + self.freq_gate_emb(freq_emb)
        
        # Spatial path: expert 4
        spatial_logits = self.spatial_gate(x) + self.spatial_gate_emb(freq_emb)
        
        # Concatenate to form 5-expert logits
        logits = torch.cat([freq_logits, spatial_logits], dim=1)  # [B, 5]
...
class DecoderBlock(nn.Module):
    def __init__(self, dim, num_heads, ffn_expansion_factor, bias, LayerNorm_type, expert_layer, complexity_scale=None,
        ...
        self.adapter = AdapterLayerFreqSpatial(
            dim, rank, 
...
class MoCEIR(nn.Module):
    def __init__(self,
                ...
                bias = False,
                rank=2,
                num_experts=5,  # Fixed to 5
                depth_type="lin",
                stage_depth=[3,2,1],
                rank_type="constant",
                topk=1,
                expert_layer=FFTAttentionFreqMask,
                with_complexity=False,
                complexity_scale="max",
                ):
        super(MoCEIR, self).__init__()
        
        assert num_experts == 5, "This model requires exactly 5 experts (4 frequency + 1 spatial)"
...
if __name__ == "__main__":
    ...
    model = MoCEIR(rank=2, num_blocks=[4,6,6,8], num_dec_blocks=[2,4,4], levels=4, dim=48, num_refinement_blocks=4, 
                   ...
                   num_experts=5, topk=1, expert_layer=FFTAttentionFreqMask).cuda()
...
def build_model(opt):
    ...
    num_experts = getattr(opt, 'num_exp_blocks', 5)  # Fixed to 5
