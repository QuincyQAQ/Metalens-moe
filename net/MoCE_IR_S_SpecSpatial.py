...
##########################################################################
## Spectral-Spatial Frequency Embedding (升级版 FrequencyEmbedding)
class FrequencyEmbedding(nn.Module):
    """
    Spectral-Spatial Embedding for metasurface reconstruction.

    设计目标：
    - 频域：通过 HighPassConv2d 强调高频/边缘，缓解高频截止问题
    - 多尺度：通过不同空间尺度的池化，建模不同尺度的频率分布
    - 通道注意力：捕获不同颜色/波段之间的退化关联（色差）
    输出：
    - [B, dim] 的频域嵌入向量，用于 MoE 的路由（与原接口保持一致）
    """
    ...
    def __init__(self, dim, num_scales: int = 3, ca_reduction: int = 4):
        ...
        # 3) 多尺度频域统计
        #    使用不同的空间池化尺度，分别对应「全局结构 / 中尺度 / 局部高频」
        #    例如: 1x1, 2x2, 4x4（可在构造时通过 num_scales 控制）
        pool_sizes = [1, 2, 4][:num_scales]
        self.pool_sizes = pool_sizes

        # 每个尺度都会产生 dim * (ps*ps) 维度，concat 后通过 MLP 压到 dim
    ...
    def forward(self, x):
        ...
        for ps in self.pool_sizes:
            # 自适应池化到 ps x ps，然后拉平
            pooled = F.adaptive_avg_pool2d(feat, (ps, ps))  # [B, C, ps, ps]
            pooled = pooled.view(b, -1)                     # [B, C*ps*ps]
            pooled_feats.append(pooled)

        multi_scale_vec = torch.cat(pooled_feats, dim=1)    # [B, multi_dim]

        # 4) MLP 融合多尺度频域信息，输出频域嵌入
        emb = self.mlp(multi_scale_vec)                     # [B, dim]
        return emb
