...
class WaveletRecon(nn.Module):
    ...
    def forward(self, cA, cH, cV, cD, target_size=None):
        ...
        for i in range(c):
            ...
            ll_up = F.conv_transpose2d(cA_ch, self.ll_kernel, stride=2, padding=0, output_padding=0)
...
class MoCEIR(nn.Module):
    def __init__(self,
        ...
        self.wavelet_projs = nn.ModuleList([
            nn.Conv2d(inp_channels, dim, kernel_size=3, padding=1, bias=False)  # 每个频段投影到dim维度
        ...
        self.wavelet_recon_projs = nn.ModuleList([
            nn.Conv2d(dim, out_channels, kernel_size=3, padding=1, bias=False)  # 投影回原始通道数
            for _ in range(4)  # LL, LH, HL, HH
        ])
    ...
    def forward(self, x, labels=None):
        ...
        freq_emb = self.freq_embed(feats)  # 使用SV频率嵌入
