...
##########################################################################
## Spatially-Variant Spectral-Spatial Frequency Embedding
## 融合了 SV (Spatially-Variant) 和 SpecSpatial (Spectral-Spatial) 两个思路
class SpatiallyVariantSpectralSpatialFreqEmbedding(nn.Module):
    """
    融合版本频率嵌入：结合空间变异感知和谱域-空间联合建模
    
    核心创新点融合：
    1. 从 SV 版本借鉴：多尺度局部池化 (1x1, 2x2, 4x4) - 捕捉空间变异（中心 vs 边缘）
    2. 从 SpecSpatial 版本借鉴：通道注意力 (Channel Attention) - 建模谱域关系（RGB/波段间的退化关联）
    
    设计流程：
    - 高通滤波提取高频细节
    - 通道注意力建模谱域关系（色差/波段退化）
    - 多尺度局部池化捕捉空间变异（区分不同空间区域的频率分布）
    - MLP融合所有特征
    
    输出：
    - [B, dim] 的频域嵌入向量，用于 MoE 的路由
    """
    def __init__(self, dim, num_scales: int = 3, ca_reduction: int = 4):
        super(SpatiallyVariantSpectralSpatialFreqEmbedding, self).__init__()
        self.dim = dim
        self.num_scales = num_scales

        # 1) 高通滤波 + 非线性：提取高频细节（像差最敏感的部分）
        #    与 baseline 和两个变体保持一致
        self.high_conv = nn.Sequential(
            HighPassConv2d(dim, freeze=True),
            nn.GELU()
        )
        
        # 2) 通道注意力（SE/CBAM 风格）- 来自 SpecSpatial 版本
        #    让网络自动学习不同通道（RGB 或多波段）之间的相关性和重要性
        #    建模色差/波段退化等谱域关系
        mid_channels = max(dim // ca_reduction, 1)
        self.channel_attn = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),             # [B, C, 1, 1]
            nn.Conv2d(dim, mid_channels, 1, 1),  # 降维
            nn.GELU(),
            nn.Conv2d(mid_channels, dim, 1, 1),   # 升维
            nn.Sigmoid(),
        )
        ...
        #    例如: 1x1 (全局), 2x2 (四个象限), 4x4 (细粒度区域)
        pool_sizes = [1, 2, 4][:num_scales]
        self.pool_sizes = pool_sizes
        self.local_pools = nn.ModuleList([
            nn.AdaptiveAvgPool2d(ps) for ps in pool_sizes
        ])

        # 4) 特征融合 MLP
        #    每个尺度产生 dim * (ps*ps) 维度，concat 后通过 MLP 压缩到 dim
        #    1*1 + 2*2 + 4*4 = 1 + 4 + 16 = 21 个空间位置（如果 num_scales=3）
        multi_dim = sum((ps * ps) for ps in pool_sizes) * dim
        hidden_dim = max(2 * dim, dim + 1)

        self.fusion = nn.Sequential(
            nn.Linear(multi_dim, hidden_dim),
            nn.GELU(),
    ...
    def forward(self, x):
        """
        x: [B, C=dim, H, W] - 来自 bottleneck 的特征
        return: [B, dim] - 频率嵌入向量
        """
        b, c, h, w = x.shape

        # 1) 高通特征提取
        feat = self.high_conv(x)  # [B, C, H, W]

        # 2) 通道注意力：针对每个通道自适应加权，建模色差/波段退化
        #    来自 SpecSpatial 版本的核心创新
        ca_weight = self.channel_attn(feat)      # [B, C, 1, 1]
        feat = feat * ca_weight                  # [B, C, H, W]

        # 3) 多尺度局部池化：提取不同空间区域的频率特征
        #    来自 SV 版本的核心创新
        #    能够区分"中心 vs 边缘"等空间变异
        pooled_feats = []
        for pool in self.local_pools:
            f = pool(feat)  # [B, C, ps, ps]
            f = f.flatten(1)  # [B, C * ps * ps]
            pooled_feats.append(f)

        # 4) 拼接并融合所有多尺度特征
        combined = torch.cat(pooled_feats, dim=1)  # [B, multi_dim]
...
class MoCEIR(nn.Module):
    def __init__(self,
        ...
        # 使用融合版本的频率嵌入
        self.freq_embed = SpatiallyVariantSpectralSpatialFreqEmbedding(dims[-1])
