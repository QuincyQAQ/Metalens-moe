"""
MoCE-IR with Three Innovations Fusion (CGM-CR-EI Version)
融合三个创新点的完整网络实现（因果生成模型驱动版本）

创新点1: 空间变异频率诊断 (Spatially-Variant Frequency Diagnosis, SVFD)
- SpatiallyVariantFreqEmbedding: 通过多尺度局部池化显式建模空间变异，使用高通滤波器提取像差敏感的高频区域

创新点2: 多模态物理先验融合专家 (Multi-Modal Physics-Prior Fusion Experts, PGHC)
- HeteroExpert: 支持物理先验融合的异构专家（深度、光谱、光学参数）

创新点3: 因果生成模型驱动的逆事实路由与专家干预 (Causal Generative Model-driven Counterfactual Routing with Expert Intervention, CGM-CR-EI)
- CGM_CR_Router: 因果生成模型路由器，通过逆事实推理选择最优专家
- ExpertIntervention: 专家干预模块，每个专家针对特定因果退化因子进行干预

三个创新点形成"诊断-会诊-因果推理"闭环：
1. SVFD (诊断): 空间变异频率诊断，识别不同区域的退化模式
2. PGHC (会诊): 多模态物理先验融合，不同专家利用不同物理先验进行针对性修复
3. CGM-CR-EI (因果推理): 通过因果生成模型进行逆事实推理，选择最优专家进行干预
"""
...
class AdapterLayer(nn.Module):
    def __init__(self, 
        ...
        self.routing = CGM_CR_Router(
            ...
            img_size=256  # 可以根据实际输入调整
        )
        
    def forward(self, x, freq_emb, shared, raw_physics_priors=None, degraded_image=None, clean_image=None):
        # CGM-CR路由器需要退化图像和清晰图像（用于训练）
        gates, top_k_indices, top_k_values, aux_loss, encoded_physics_priors = self.routing(
            x, freq_emb, raw_physics_priors, degraded_image=degraded_image, clean_image=clean_image
        )
        self.loss = aux_loss
...
class EncoderCGM(nn.Module):
    """
    因果生成模型的编码器：推断潜在因果退化因子
    """
    def __init__(self, in_channels_img, in_channels_svfe, latent_dim_c, latent_dim_r, img_size=256):
        super().__init__()
        self.conv1_img = nn.Conv2d(in_channels_img, 64, kernel_size=4, stride=2, padding=1)
        self.conv1_svfe = nn.Conv2d(in_channels_svfe, 64, kernel_size=4, stride=2, padding=1)
        ...
        # Assuming input image is 256x256, after 4 stride-2 convs, spatial dim becomes 256/16 = 16
        initial_spatial_dim = img_size // 16
        self.flattened_size = 512 * initial_spatial_dim * initial_spatial_dim
        
        self.fc_mu_c = nn.Linear(self.flattened_size, latent_dim_c)
        self.fc_logvar_c = nn.Linear(self.flattened_size, latent_dim_c)
        self.fc_mu_r = nn.Linear(self.flattened_size, latent_dim_r)
        self.fc_logvar_r = nn.Linear(self.flattened_size, latent_dim_r)

    def forward(self, img_degraded, svfe_features):
        h_img = F.leaky_relu(self.conv1_img(img_degraded), 0.2)
        h_svfe = F.leaky_relu(self.conv1_svfe(svfe_features), 0.2)
        
        h = torch.cat([h_img, h_svfe], dim=1)  # Concatenate features
        ...
        return mu_c, logvar_c, mu_r, logvar_r


class DecoderCGM(nn.Module):
    """
    因果生成模型的解码器：根据干预信号生成逆事实图像
    """
    def __init__(self, out_channels_img, latent_dim_c, latent_dim_r, num_experts, initial_spatial_dim=16):
        super().__init__()
        self.initial_spatial_dim = initial_spatial_dim
        self.fc_in = nn.Linear(latent_dim_c + latent_dim_r + num_experts, 512 * initial_spatial_dim * initial_spatial_dim)
        
        self.deconv1 = nn.ConvTranspose2d(512, 256, kernel_size=4, stride=2, padding=1)
        self.deconv2 = nn.ConvTranspose2d(256, 128, kernel_size=4, stride=2, padding=1)
        self.deconv3 = nn.ConvTranspose2d(128, 64, kernel_size=4, stride=2, padding=1)
        self.deconv4 = nn.ConvTranspose2d(64, out_channels_img, kernel_size=4, stride=2, padding=1)

    def forward(self, z_c, z_r, expert_intervention_signal):
        # expert_intervention_signal is one-hot or embedding for the expert
        z = torch.cat([z_c, z_r, expert_intervention_signal], dim=1)
        h = self.fc_in(z)
        h = h.view(h.size(0), 512, self.initial_spatial_dim, self.initial_spatial_dim)
        
        h = F.relu(self.deconv1(h))
        h = F.relu(self.deconv2(h))
        h = F.relu(self.deconv3(h))
        img_predicted = torch.tanh(self.deconv4(h))  # Output image in [-1, 1] range
        
        return img_predicted


class CGM_CR_Router(nn.Module):
    """
    因果生成模型驱动的逆事实路由：通过逆事实推理选择最优专家
    """
    def __init__(self, dim, freq_dim, num_experts, k, complexity, use_complexity_bias: bool = True, 
                 complexity_scale: str="max", use_physics_prior: bool=False, H=None, W=None, 
                 latent_dim_c=16, latent_dim_r=64, img_size=256):
        super(CGM_CR_Router, self).__init__()
        ...
        # CGM编码器和解码器
        # 假设输入图像通道数为3，SVFE特征通道数为dim
        in_channels_img = 3  # RGB
        in_channels_svfe = dim
        self.encoder_cgm = EncoderCGM(in_channels_img, in_channels_svfe, latent_dim_c, latent_dim_r, img_size)
        self.decoder_cgm = DecoderCGM(in_channels_img, latent_dim_c, latent_dim_r, num_experts, 
                                      initial_spatial_dim=img_size // 16)
        ...
        if use_physics_prior:
            ...
            self.physics_confidence_predictor = PhysicsConfidencePredictor(dim_physics_feat=dim // 4, 
                                                                          num_physics_priors=self.num_physics_priors)
        else:
        ...
        # 频率嵌入到专家权重的映射（用于推理时的备用选择）
        self.freq_gate = nn.Linear(freq_dim, num_experts, bias=False)
        
        # 用于存储逆事实图像（用于训练）
        self.counterfactual_images = None
        self.selected_expert_indices = None
    ...
    def forward(self, x, freq_emb, raw_physics_priors=None, degraded_image=None, clean_image=None):
        """
        Args:
            x: 特征图 (B, C, H, W)
            freq_emb: 频率嵌入 (B, freq_dim)
            raw_physics_priors: 物理先验字典
            degraded_image: 退化图像 (B, 3, H, W) - 用于CGM
            clean_image: 清晰图像 (B, 3, H, W) - 用于训练时的逆事实评估
        """
        B, C, H, W = x.shape
        
        # 如果degraded_image未提供，使用x作为替代（需要调整通道数）
        if degraded_image is None:
            # 将特征图转换为3通道图像用于CGM（简化处理）
            if C != 3:
                # 先调整空间尺寸
                degraded_image = F.adaptive_avg_pool2d(x, (self.img_size, self.img_size))
                if C > 3:
                    # 取前3个通道
                    degraded_image = degraded_image[:, :3, :, :]
                elif C < 3:
                    # 如果通道数少于3，用0填充
                    degraded_image = F.pad(degraded_image, (0, 0, 0, 0, 0, 3 - C))
            else:
                degraded_image = F.adaptive_avg_pool2d(x, (self.img_size, self.img_size))
        
        # 确保degraded_image的尺寸正确
        if degraded_image.shape[2] != self.img_size or degraded_image.shape[3] != self.img_size:
            degraded_image = F.interpolate(degraded_image, size=(self.img_size, self.img_size), mode='bilinear', align_corners=False)
        
        # 确保degraded_image是3通道
        if degraded_image.shape[1] != 3:
            if degraded_image.shape[1] > 3:
                degraded_image = degraded_image[:, :3, :, :]
            else:
                degraded_image = F.pad(degraded_image, (0, 0, 0, 0, 0, 3 - degraded_image.shape[1]))
        
        # 准备SVFE特征（使用x作为SVFE特征）
        svfe_features = F.adaptive_avg_pool2d(x, (self.img_size, self.img_size))
        if svfe_features.shape[1] != C:
            svfe_features = F.interpolate(svfe_features, size=(self.img_size, self.img_size), mode='bilinear', align_corners=False)
        
        # 1. 编码到潜在因果因子
        mu_c, logvar_c, mu_r, logvar_r = self.encoder_cgm(degraded_image, svfe_features)
        
        if self.training:
            z_c = self.reparameterize(mu_c, logvar_c)
            z_r = self.reparameterize(mu_r, logvar_r)
        else:
            z_c = mu_c  # Use mean during inference
            z_r = mu_r
        
        # 2. 逆事实推理：为每个专家模拟干预（向量化处理以避免DDP问题）
        # 创建所有专家的one-hot信号: (num_experts, num_experts)
        expert_signals = F.one_hot(torch.arange(self.num_experts, device=degraded_image.device), num_classes=self.num_experts).float()
        # 扩展为 (num_experts * B, num_experts)
        expert_signals_expanded = expert_signals.unsqueeze(1).repeat(1, B, 1).view(self.num_experts * B, self.num_experts)
        
        # 扩展z_c和z_r以匹配批量大小: (num_experts * B, latent_dim)
        z_c_expanded = z_c.unsqueeze(0).repeat(self.num_experts, 1, 1).view(self.num_experts * B, -1)
        z_r_expanded = z_r.unsqueeze(0).repeat(self.num_experts, 1, 1).view(self.num_experts * B, -1)
        
        # 一次性批量生成所有专家的逆事实图像: (num_experts * B, 3, H, W)
        all_counterfactual_images = self.decoder_cgm(z_c_expanded, z_r_expanded, expert_signals_expanded)
        
        # 分割回每个专家的图像列表: [(B, 3, H, W), ...]
        counterfactual_images = [all_counterfactual_images[i * B:(i + 1) * B] for i in range(self.num_experts)]
        
        self.counterfactual_images = counterfactual_images
        
        # 3. 评估并选择最优专家（向量化处理以避免DDP问题）
        # 统一计算freq_scores，避免在DDP中重复标记参数
        freq_scores = self.freq_gate(freq_emb)  # (B, num_experts)
        
        if self.training and clean_image is not None:
            # 检查clean_image的维度
            if clean_image.dim() < 3:
                # 如果维度不足，跳过逆事实评估，使用频率嵌入作为代理指标
                selected_expert_indices = torch.argmax(freq_scores, dim=1)
            else:
                # 调整clean_image尺寸以匹配逆事实图像
                clean_image_resized = F.adaptive_avg_pool2d(clean_image, (self.img_size, self.img_size))
                
                # 向量化计算所有专家对所有样本的L1距离
                # counterfactual_images: [(B, 3, H, W), ...] -> stack to (num_experts, B, 3, H, W)
                cf_images_stack = torch.stack(counterfactual_images, dim=0)  # (num_experts, B, 3, H, W)
                # clean_image_resized: (B, 3, H, W) -> expand to (num_experts, B, 3, H, W)
                clean_expanded = clean_image_resized.unsqueeze(0).expand(self.num_experts, -1, -1, -1, -1)
                # 计算L1距离: (num_experts, B)
                l1_distances = F.l1_loss(cf_images_stack, clean_expanded, reduction='none').mean(dim=(2, 3, 4))
                # 选择L1距离最小的专家（即score最大的）
                selected_expert_indices = torch.argmin(l1_distances, dim=0)  # (B,)
        else:
            # 推理时，使用频率嵌入作为代理指标
            selected_expert_indices = torch.argmax(freq_scores, dim=1)
        
        self.selected_expert_indices = selected_expert_indices
        
        # 准备路由权重（one-hot for hard routing）
        routing_weights = F.one_hot(selected_expert_indices, num_classes=self.num_experts).float()
        ...
        if self.training:
            # KL散度损失
            kl_loss_c = -0.5 * torch.sum(1 + logvar_c - mu_c.pow(2) - logvar_c.exp())
            kl_loss_r = -0.5 * torch.sum(1 + logvar_r - mu_r.pow(2) - logvar_r.exp())
            kl_loss = (kl_loss_c + kl_loss_r) / B
            
            # 逆事实预测损失（如果clean_image可用，向量化处理以避免DDP问题）
            cf_pred_loss = 0
            if clean_image is not None and clean_image.dim() >= 3:
                clean_image_resized = F.adaptive_avg_pool2d(clean_image, (self.img_size, self.img_size))
                # 向量化选择每个样本对应的最佳逆事实图像
                # counterfactual_images: [(B, 3, H, W), ...] -> stack to (num_experts, B, 3, H, W)
                cf_images_stack = torch.stack(counterfactual_images, dim=0)  # (num_experts, B, 3, H, W)
                # 使用gather选择每个样本对应的专家图像: (B, 3, H, W)
                best_cf_images = cf_images_stack[selected_expert_indices, torch.arange(B, device=degraded_image.device)]
                # 向量化计算L1损失
                cf_pred_loss = F.l1_loss(best_cf_images, clean_image_resized)
            
            # 重要性损失和负载均衡损失（简化版）
            gating_scores = routing_weights
            loss_imp = self.importance_loss(gating_scores)
            loss_load = self.load_loss_simple(routing_weights)
            
            aux_loss = 0.5 * loss_imp + 0.5 * loss_load + 0.001 * kl_loss + 0.1 * cf_pred_loss
        else:
            aux_loss = 0
        
        # 为了兼容性，返回top_k格式
        top_k_values = routing_weights.max(dim=1)[0].unsqueeze(1)
        top_k_indices = selected_expert_indices.unsqueeze(1)
        
        return routing_weights, top_k_indices, top_k_values, aux_loss, encoded_physics_priors
    ...
    def load_loss_simple(self, routing_weights):
        # 简化的负载均衡损失
        load_per_expert = routing_weights.sum(dim=0)
        load_mean = load_per_expert.mean()
        load_std = load_per_expert.std()
        loss_load = (load_std / (load_mean + 1e-8)) ** 2
        return loss_load
    


##########################################################################
## 创新点2: 自适应物理约束路由（保留作为备用，但主要使用CGM_CR_Router）
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, freq_emb=None, raw_physics_priors=None, degraded_image=None, clean_image=None):    
        shortcut = x
        x = self.norms[0](x)
        ...
        x_a = self.proj[1](x)
        x_s = self.shared(x_s)
        x_a = self.adapter(x_a, freq_emb, x_s, raw_physics_priors, degraded_image=degraded_image, clean_image=clean_image)
...
class DecoderResidualGroup(nn.Module):
    ...
    def forward(self, x, freq_emb=None, raw_physics_priors=None, degraded_image=None, clean_image=None):
        ...
        self.loss = 0
        while i < len(self.layers):
            x , loss = self.layers[i](x, freq_emb, raw_physics_priors, degraded_image=degraded_image, clean_image=clean_image)
            self.loss += loss
            i += 1
        return x  
...
class MoCEIR(nn.Module):
    ...
    def forward(self, x, labels=None, raw_physics_priors=None):
        ...
        for i, (upsample, fusion, block) in enumerate(self.dec):
            ...
            feats = block(feats, freq_emb, adjusted_physics_priors, degraded_image=x, clean_image=labels)
