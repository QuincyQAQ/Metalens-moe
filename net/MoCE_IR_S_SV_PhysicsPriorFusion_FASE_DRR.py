...
##########################################################################
## 创新点3: FASE-DRR - 频率感知专家共享与动态残差路由
## 1. 频率感知路由器 (Frequency-Aware Router, FAR)
class FrequencyAwareRouter(nn.Module):
    """
    频率感知路由器：使用FFT提取频率特征，结合SVFE和PhysicsPriorFusion特征进行路由决策
    """
    def __init__(self, dim, freq_dim, num_specialized_experts, svfe_feature_dim=None, prior_dim=None, hidden_dim=128):
        super().__init__()
        self.num_specialized_experts = num_specialized_experts
        
        # 如果svfe_feature_dim和prior_dim未指定，使用默认值
        if svfe_feature_dim is None:
            svfe_feature_dim = dim
        ...
        # 融合所有特征的MLP
        input_dim = svfe_feature_dim + prior_dim + fft_feature_dim
        self.router_mlp = nn.Sequential(
            nn.Linear(input_dim, hidden_dim),
            nn.ReLU(),
            nn.Linear(hidden_dim, hidden_dim // 2),
            nn.ReLU(),
            nn.Linear(hidden_dim // 2, num_specialized_experts)  # 输出每个特化专家的logits
        )
    
    def _extract_fft_features(self, image):
        """
        从图像中提取FFT频率特征
        image: (B, C, H, W)
        """
        ...
        fft_magnitude = torch.abs(torch.fft.fftshift(fft_image, dim=(-2, -1)))
        
        # 池化得到特征向量
        fft_features = F.adaptive_avg_pool2d(fft_magnitude, (1, 1)).squeeze(-1).squeeze(-1)
        return fft_features  # (B, C)
    
    def forward(self, degraded_image, svfe_features_map, physics_prior):
        """
        Args:
            degraded_image: (B, C, H, W) 退化图像
            svfe_features_map: (B, svfe_feature_dim, H, W) SVFE特征图
            physics_prior: (B, prior_dim) 物理先验特征向量
        Returns:
            residual_weights: (B, num_specialized_experts) 动态残差路由权重
        """
        batch_size = degraded_image.size(0)
        
        # 提取FFT特征
        fft_features = self._extract_fft_features(degraded_image)  # (B, C)
        
        # 池化SVFE特征为向量
        svfe_features_vec = F.adaptive_avg_pool2d(svfe_features_map, (1, 1)).squeeze(-1).squeeze(-1)  # (B, svfe_feature_dim)
        
        # 拼接所有特征
        combined_features = torch.cat([svfe_features_vec, physics_prior, fft_features], dim=1)
        
        # 获取原始logits
        raw_logits = self.router_mlp(combined_features)
        
        # 使用Sigmoid生成动态残差路由权重（允许多个专家同时激活）
        residual_weights = torch.sigmoid(raw_logits)  # (B, num_specialized_experts)
        
        return residual_weights


## 2. 共享-特化专家架构 (Shared-Specialized Expert Architecture, SSEA)
...
########################################################################### 
## Adapter Layer (FASE-DRR版本：支持频率感知专家共享与动态残差路由)
class AdapterLayer(nn.Module):
    def __init__(self, 
        ...
        self.num_specialized_experts = num_experts  # 特化专家数量等于原专家数量
        
        # 计算共享专家和特化专家的参数
        patch_size_shared = 4  # 共享专家使用中等patch size
        kernel_size_shared = 5
        depth_shared = stage_depth + 2  # 共享专家深度稍大
        rank_shared = rank
        
        # 特化专家参数（轻量级）
        patch_sizes = [2**(i+2) for i in range(self.num_specialized_experts)]
        kernel_sizes = [3+(2*i) for i in range(self.num_specialized_experts)]
        
        if depth_type == "lin":
            depths = [stage_depth+i for i in range(self.num_specialized_experts)]
        elif depth_type == "double":
            depths = [stage_depth+(2*i) for i in range(self.num_specialized_experts)]
        elif depth_type == "exp":
            depths = [2**(i) for i in range(self.num_specialized_experts)]
        elif depth_type == "fact":
            depths = [math.factorial(i+1) for i in range(self.num_specialized_experts)]
        elif isinstance(depth_type, int):
            depths = [depth_type for _ in range(self.num_specialized_experts)]
        elif depth_type == "constant":
            depths = [stage_depth for i in range(self.num_specialized_experts)]
        else:
            raise(NotImplementedError)
        
        if rank_type == "constant":
            ranks = [rank for _ in range(self.num_specialized_experts)]
        elif rank_type == "lin":
            ranks = [rank+i for i in range(self.num_specialized_experts)]
        elif rank_type == "double":
            ranks = [rank+(2*i) for i in range(self.num_specialized_experts)]
        elif rank_type == "exp":
            ranks = [rank**(i+1) for i in range(self.num_specialized_experts)]
        elif rank_type == "fact":
            ranks = [math.factorial(rank+i) for i in range(self.num_specialized_experts)]
        elif rank_type == "spread":
            ranks = [dim//(2**i) for i in range(self.num_specialized_experts)][::-1]
        else:
            raise(NotImplementedError)
        
        # 1. 共享专家（处理通用退化）
        self.shared_expert = MySequential(*[SharedExpert(
            dim, rank=rank_shared, func=expert_layer, 
            depth=depth_shared, patch_size=patch_size_shared, kernel_size=kernel_size_shared
        )])
        
        # 2. 特化专家（轻量级，专注于残差修正）
        self.specialized_experts = nn.ModuleList([
            MySequential(*[SpecializedExpert(
                dim, rank=r, func=expert_layer, 
                depth=d, patch_size=p, kernel_size=k
            )])
            for d, r, p, k in zip(depths, ranks, patch_sizes, kernel_sizes)
        ])
        
        # 3. 频率感知路由器（FAR）
        # 需要从RoutingFunction获取物理先验特征维度
        svfe_feature_dim = dim  # SVFE特征维度
        prior_dim = dim // 4 if use_physics_prior else 0  # 物理先验维度
        
        self.far_router = FrequencyAwareRouter(
            dim=dim,
            freq_dim=freq_dim,
            num_specialized_experts=self.num_specialized_experts,
            svfe_feature_dim=svfe_feature_dim,
            prior_dim=prior_dim if use_physics_prior else 0
        )
    ...
    def _encode_physics_priors(self, raw_physics_priors, B, H, W, device):
        """编码物理先验为特征向量"""
        if not self.use_physics_prior or raw_physics_priors is None:
        ...
        if "depth_map" in raw_physics_priors and raw_physics_priors["depth_map"] is not None:
            ...
            encoded_priors.append(F.adaptive_avg_pool2d(depth_feat, (1, 1)).squeeze(-1).squeeze(-1))
        else:
            encoded_priors.append(torch.zeros(B, self.depth_encoder.encoder[-2].out_channels, device=device))
        ...
        if "spectral_data" in raw_physics_priors and raw_physics_priors["spectral_data"] is not None:
            ...
            encoded_priors.append(F.adaptive_avg_pool2d(spectral_feat, (1, 1)).squeeze(-1).squeeze(-1))
        else:
            encoded_priors.append(torch.zeros(B, self.spectral_encoder.encoder[-2].out_channels, device=device))
        
        if "optical_params" in raw_physics_priors and raw_physics_priors["optical_params"] is not None:
            optical_feat = self.optical_param_encoder(raw_physics_priors["optical_params"], H=H, W=W)
            encoded_priors.append(F.adaptive_avg_pool2d(optical_feat, (1, 1)).squeeze(-1).squeeze(-1))
        else:
            encoded_priors.append(torch.zeros(B, self.optical_param_encoder.encoder[-1].out_features, device=device))
        
        # 拼接所有物理先验特征
        if len(encoded_priors) > 0:
            physics_prior = torch.cat(encoded_priors, dim=1)
        else:
            physics_prior = torch.zeros(B, 0, device=device)
        
        return physics_prior
    ...
    def forward(self, x, freq_emb, shared, raw_physics_priors=None):
        ...
        """
        B, C, H, W = x.shape
        self.loss = 0  # FASE-DRR不使用auxiliary loss
        
        # 1. 共享专家生成基线修复
        base_output = self.shared_expert(x, shared)  # (B, C, H, W)
        
        # 2. 准备频率感知路由器的输入
        # 将freq_emb扩展为特征图（用于SVFE特征）
        # 注意：freq_emb是(B, freq_dim)向量，需要转换为特征图
        # 这里我们使用x作为SVFE特征的代理（实际应该从SVFE模块获取）
        svfe_features_map = x  # 简化：使用x作为SVFE特征图
        
        # 编码物理先验
        if self.use_physics_prior and self.depth_encoder is not None:
            physics_prior = self._encode_physics_priors(raw_physics_priors, B, H, W, x.device)  # (B, prior_dim)
        else:
            physics_prior = torch.zeros(B, 0, device=x.device)
        
        # 3. 频率感知路由器生成动态残差路由权重
        residual_weights = self.far_router(x, svfe_features_map, physics_prior)  # (B, num_specialized_experts)
        
        # 4. 特化专家生成残差修正
        all_residual_maps = []
        for i in range(self.num_specialized_experts):
            residual_map = self.specialized_experts[i](x, shared)  # (B, C, H, W)
            all_residual_maps.append(residual_map)
        
        # 堆叠所有残差图: (B, num_specialized_experts, C, H, W)
        all_residual_maps_tensor = torch.stack(all_residual_maps, dim=1)
        
        # 5. 动态残差路由（DRR）：加权叠加残差修正
        # 扩展residual_weights以匹配残差图的维度
        residual_weights_expanded = residual_weights.unsqueeze(2).unsqueeze(3).unsqueeze(4)  # (B, num_specialized_experts, 1, 1, 1)
        
        # 加权残差和
        weighted_residual_sum = torch.sum(residual_weights_expanded * all_residual_maps_tensor, dim=1)  # (B, C, H, W)
        
        # 6. 最终输出：基线修复 + 加权残差修正
        out = base_output + weighted_residual_sum
