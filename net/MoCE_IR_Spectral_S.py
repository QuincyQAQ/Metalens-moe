...
class SpectralExpert(nn.Module):
    ...
    def forward(self, x):
        in_dtype = x.dtype
        # 确保 FFT 操作在 float32 下进行，避免 half precision 下 cuFFT 的限制
        # cuFFT 在 half precision 下只支持 2 的幂次方尺寸
        # 使用 torch.cuda.amp.autocast(enabled=False) 确保禁用自动混合精度
        with torch.cuda.amp.autocast(enabled=False):
            x_float = x.float()
            fft_x = torch.fft.rfft2(x_float, dim=(-2, -1), norm="ortho")
            fft_x_split = torch.cat([fft_x.real, fft_x.imag], dim=1)
            processed_fft_split = self.spectral_filter(fft_x_split.float())
...
if __name__ == "__main__":
    # test - S版本使用更小的配置
    model = MoCEIR(rank=2, num_blocks=[2,4,4,6], num_dec_blocks=[1,2,2], levels=4, dim=32, num_refinement_blocks=2, 
                   with_complexity=True, complexity_scale="max", stage_depth=[1,1,1], depth_type="constant", rank_type="spread", 
                   num_experts=3, topk=1, expert_layer=FFTAttention).cuda()
...
def build_model(opt):
    ...
    """
    # S版本默认参数：更小的模型
    dim = getattr(opt, 'dim', 32)  # 从48降到32
    num_blocks = getattr(opt, 'num_blocks', [2, 4, 4, 6])  # 从[4,6,6,8]减少
    num_dec_blocks = getattr(opt, 'num_dec_blocks', [1, 2, 2])  # 从[2,4,4]减少
    heads = getattr(opt, 'heads', [1, 1, 2, 4])  # 从[1,2,4,8]减少
    num_refinement_blocks = getattr(opt, 'num_refinement_blocks', 2)  # 从4降到2
    topk = getattr(opt, 'topk', 1)
    num_experts = getattr(opt, 'num_exp_blocks', 3)  # 从4降到3
