...
###########################################################################
## Adapter Layer (Physics-Informed Routing)
class AdapterLayer(nn.Module):
    """
    与 MoCE_IR_S 的 AdapterLayer 结构保持一致，但 RoutingFunction 使用物理启发式门控：
    - 在特征上拼接坐标 (x, y, r)，其中 r 为到光轴的距离
    - 结合频域嵌入 freq_emb 作为附加 gate
    这样专家选择同时依赖图像内容 + 空间位置 (近轴/远轴) + 频域信息。
    """
    ...
    def __init__(
        ...
        self.experts = nn.ModuleList(
            [
                ...
                for idx, (depth, rank, patch, kernel) in enumerate(
                    zip(depths, ranks, patch_sizes, kernel_sizes)
                )
            ]
        )
        ...
        expert_complexity = torch.tensor(
            [sum(p.numel() for p in expert.parameters()) for expert in self.experts]
        )
    ...
    def forward(self, x, freq_emb, shared):
        ...
        if self.training:
            ...
            expert_outputs = [
                self.experts[exp](expert_inputs[exp], expert_shared_intputs[exp])
                for exp in range(len(self.experts))
            ]
            out = dispatcher.combine(expert_outputs, multiply_by_gates=True)
        else:
            selected_experts = [
                self.experts[i] for i in top_k_indices.squeeze(0)
            ]  # Select the corresponding experts
            expert_outputs = torch.stack(
                [expert(x, shared) for expert in selected_experts], dim=1
            )
            gates = gates.gather(1, top_k_indices)
            weighted_outputs = (
                gates.unsqueeze(2).unsqueeze(3).unsqueeze(4) * expert_outputs
            )
            out = weighted_outputs.sum(
                dim=1
            )  # Sum across the top-k dimension to get the final output
...
class RoutingFunction(nn.Module):
    """
    Physics-Informed Gating:
    - 在通道上拼接 (x, y, r) 三个坐标通道，其中 r 为归一化到 [-1,1] 平面上的极径
    - 用一个 3x3 Conv + ReLU + GAP 的小 CNN 提取物理空间特征，再线性映射到专家维度
    - 再加上 freq_gate(freq_emb)（频域信息）形成最终 logits
    这样专家选择不仅依赖内容，还显式依赖到光轴的距离 r（越远越可能色差严重、PSF 展宽）。
    """
    ...
    def __init__(
        ...
        # 物理+内容的 gate（参考 MoCE_IR_PhysRouting）
        self.gate = nn.Sequential(
            nn.Conv2d(dim + self.coord_dim, 64, kernel_size=3, padding=1, bias=True),
            nn.ReLU(inplace=True),
            nn.AdaptiveAvgPool2d(1),
            Rearrange("b c 1 1 -> b c"),
            nn.Linear(64, num_experts, bias=False),
        )
        # 频域 gate：来自 bottleneck 的 FrequencyEmbedding
    ...
    def _get_coords(self, x: torch.Tensor) -> torch.Tensor:
        ...
        生成 (x, y, r) 坐标图：
        - x, y: 在 [-1, 1] 范围
        - r = sqrt(x^2 + y^2)，近似对应像素到光轴的归一化距离
        """
        b, _, h, w = x.shape
        key = (int(h), int(w), str(x.device), x.dtype)
        coords = self._coord_cache.get(key, None)
        if coords is None or coords.device != x.device or coords.dtype != x.dtype:
            yy_lin = torch.linspace(-1.0, 1.0, h, device=x.device, dtype=x.dtype)
            xx_lin = torch.linspace(-1.0, 1.0, w, device=x.device, dtype=x.dtype)
            try:
                yy, xx = torch.meshgrid(yy_lin, xx_lin, indexing="ij")
            except TypeError:
                yy, xx = torch.meshgrid(yy_lin, xx_lin)
            rr = torch.sqrt(xx * xx + yy * yy)
            coords = torch.stack([xx, yy, rr], dim=0).unsqueeze(0)
    ...
    def forward(self, x, freq_emb):
        ...
        # 内容 + 物理空间 gate
        logits = self.gate(x_with_coords)

        # 加上频域 gate（频率成分、PSF 相关退化）
        ...
        gates = torch.zeros_like(logits).scatter_(
            1, top_k_indices, top_k_values.to(dtype=logits.dtype)
        )
    ...
    def importance_loss(self, gating_scores):
        ...
        importance = (
            importance * (self.complexity * self.tau)
...
class EncoderBlock(nn.Module):
    def __init__(
        ...
        self.norms = nn.ModuleList(
            [LayerNorm(dim, LayerNorm_type), LayerNorm(dim, LayerNorm_type)]
        )
...
class FrequencyEmbedding(nn.Module):
    """
    Embeds high-frequency features at the bottleneck of the U-Net.
    """
...
class MoCEIR(nn.Module):
    """
    S 版本的基础结构不变，只是 Decoder 侧使用物理启发式 RoutingFunction：
    - 频域嵌入 freq_emb：表征频谱/PSF 相关退化
    - 空间坐标 (x, y, r)：表征像素到光轴的距离 r（近轴/远轴）
    专家选择由这两部分共同决定，更符合超透镜成像的物理特性。
    """
    ...
    def __init__(
        ...
        ranks = [rank for _ in range(levels - 1)]

        # -- Patch Embedding
        self.patch_embed = OverlapPatchEmbed(
            in_c=inp_channels, embed_dim=dim, bias=False
        )
            ...
            self.dec.append(
                nn.ModuleList(
                    [
                        Upsample(dims[i]),
                        nn.Conv2d(
                            dims[i], dims[i + 1], kernel_size=1, bias=bias
                        ),
