...
class Freq_LKA_Expert(nn.Module):
    ...
    def forward(self, x: torch.Tensor, phys_emb: torch.Tensor) -> torch.Tensor:
        ...
        x_main, gate_raw = x_and_gate.chunk(2, dim=1)

        # === DEBUG: 打印 gate 原始分布 ===
        if self.DEBUG_GATE:
            print(f"\n[Gate Raw] mean={gate_raw.mean().item():.6f}, std={gate_raw.std().item():.6f}, "
                  f"(>0)={(gate_raw>0).float().mean().item():.4f}, "
                  f"min={gate_raw.min().item():.6f}, max={gate_raw.max().item():.6f}")
        ...
        # === DEBUG: 打印 p_gate 分布 ===
        if self.DEBUG_GATE:
            print(f"[P_Gate]   mean={p_gate.mean().item():.6f}, std={p_gate.std().item():.6f}, "
                  f"(>0)={(p_gate>0).float().mean().item():.4f}, "
                  f"min={p_gate.min().item():.6f}, max={p_gate.max().item():.6f}")

        gate = gate_raw * p_gate
        
        # === DEBUG: 打印 gelu(gate) 分布 ===
        gate_gelu = F.gelu(gate)
        if self.DEBUG_GATE:
            print(f"[gelu(g)]  mean={gate_gelu.mean().item():.6f}, var={gate_gelu.var().item():.6e}, "
                  f"(>0)={(gate_gelu>0).float().mean().item():.4f}")

        # 7. 协同门控融合 (Co-Gating Fusion)
        x_out = x_fused * gate_gelu
        
        # === DEBUG: 打印输出幅值比 ===
        if self.DEBUG_GATE:
            x_fused_norm = x_fused.norm()
            x_out_norm = x_out.norm()
            var_ratio = x_out.var() / (x_fused.var() + 1e-8)
            print(f"[Magnitude] ||x_out||/||x_fused|| = {x_out_norm.item()/(x_fused_norm.item()+1e-8):.6f}")
            print(f"[Var Ratio] var_out/var_fused = {var_ratio.item():.6e}")
