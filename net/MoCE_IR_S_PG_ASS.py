...
class PG_ASS_Expert(nn.Module):
    ...
    def forward(self, x: torch.Tensor, phys_emb: torch.Tensor) -> torch.Tensor:
        ...
        # 2. 物理引导序列建模 (各向异性扫描)
        # 先进行空间下采样，减少序列长度，提高效率
        # 将 2D 特征转为序列 [B, H*W, C]
        x_seq = rearrange(x, 'b c h w -> b (h w) c')
        
        # 空间下采样：每 4x4 区域采样一次，将序列长度从 H*W 减少到 (H/4)*(W/4)
        # 对于 256x256 输入，序列长度从 65536 减少到 4096
        h_new, w_new = h // 4, w // 4
        x_seq_reshaped = x_seq.view(b, h, w, -1)
        x_seq_down = x_seq_reshaped[:, ::4, ::4, :].contiguous().view(b, h_new * w_new, -1)
        
        x_seq_down = self.pg_ass(x_seq_down, phys_emb)
        x_seq_down = rearrange(x_seq_down, 'b (h w) c -> b c h w', h=h_new, w=w_new)
        
        # 上采样回原始尺寸
        x_seq = F.interpolate(x_seq_down, size=(h, w), mode='bilinear', align_corners=False)
