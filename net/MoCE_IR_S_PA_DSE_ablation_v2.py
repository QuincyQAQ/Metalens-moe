...
##########################################################################
## 核心创新模块：物理感知双流专家 (PA-DSE) - 消融版本
## Physics-Aware Dual-Stream Expert - Ablation Version
##
## 学术叙事逻辑：
## 本版本在原有的 LKE 专家（不含物理门控）基础上，引入了 PG-ASME 专家。
## 实现了“空间域大核建模”与“序列域物理引导 Mamba”的协同。
## 创新点1: LKE 专家 (大核卷积捕获非等晕空间相关性)
## 创新点2: PG-ASME 专家 (物理引导各向异性扫描 Mamba 捕获各向异性退化)
##########################################################################
...
class LKE_Expert_Baseline(nn.Module):
    """
    [学术定义]: Field-Aware Co-Gating Expert (FACE) / 视场感知协同门控专家
    
    核心逻辑: 融合 LKA 的全局空间感知。
    本版本为消融版本，不含物理先验驱动的动态门控。
    """
    ...
    def forward(self, x: torch.Tensor, phys_emb: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            x: [B, C, H, W] - 图像内容特征
            phys_emb: [B, phys_dim] - 物理先验嵌入 (Physics Prior Embedding) - 在此专家中不使用
        """
        b, c, h, w = x.shape
...
class PA_DSE_Expert_Ablation(nn.Module):
    """
    [学术定义]: Physics-Aware Dual-Stream Expert (PA-DSE) / 物理感知双流专家 - 消融版本
    
    核心逻辑: 融合 LKE_Expert_Baseline 的空间域建模与 PG-ASME 的物理引导各向异性 Mamba 序列建模。
    ...
    def __init__(self, dim: int, phys_dim: int, expansion_ratio: float = 2.0, hidden_dim_asme: int = 256):
        ...
        # 空间域专家 (LKE_Expert_Baseline) - 不含物理门控
        self.lke_expert = LKE_Expert_Baseline(dim, expansion_ratio=expansion_ratio)
        
        # 物理引导各向异性 Mamba 序列专家 (PG-ASME)
    ...
    def forward(self, x: torch.Tensor, phys_emb: torch.Tensor) -> torch.Tensor:
        # 空间流处理
        lke_out = self.lke_expert(x, phys_emb) # phys_emb 在 LKE_Expert_Baseline 中不使用
...
class LKE_Router_Baseline(nn.Module):
    """
    [学术定义]: Physics-Prior Guided Router (PPGR) / 物理先验引导路由器 - 消融版本
    
    创新点: 解决了传统 MoE 路由器的“空间盲目性 (Routing Blindness)”。
    本版本为消融版本，不含物理因果分支，仅基于内容进行路由。
    """
    ...
    def forward(
        self, x: torch.Tensor, phys_emb: Optional[torch.Tensor] = None
...
class PA_DSE_AdapterLayer_Ablation(nn.Module):
    """
    [学术定义]: Physics-Aware Dual-Stream Mixture-of-Experts (PA-DMoE) Layer - 消融版本
    ...
    def __init__(self, dim: int, phys_dim: int, num_experts: int = 4, k: int = 1):
        ...
        self.experts = nn.ModuleList(
            [PA_DSE_Expert_Ablation(dim, phys_dim) for _ in range(num_experts)]
        )
        # 物理引导路由器 - 消融版本，仅内容路由
        self.router = LKE_Router_Baseline(dim, num_experts, k)

        self.out_proj = nn.Conv2d(dim, dim, kernel_size=1, bias=False)

    def forward(self, x: torch.Tensor, phys_emb: torch.Tensor) -> torch.Tensor:
        b, c, h, w = x.shape
        scores, indices, top_k_scores = self.router(x, phys_emb) # phys_emb 在 LKE_Router_Baseline 中不使用
...
class MoCEIR_PA_DSE_Ablation(nn.Module):
    """
    [学术定义]: Physics-Informed Mixture-of-Dual-Stream-Experts for Image Restoration - 消融版本
    
    架构描述: 
    1. 采用对称的 Encoder-Decoder 结构作为主干。
    2. 在 Decoder 阶段引入 PA-DMoE_Ablation 层，实现对多尺度物理退化的自适应修复。
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.pa_dmoe2 = PA_DSE_AdapterLayer_Ablation(dim * 4, phys_dim, num_experts=num_experts, k=topk)

        self.up1 = Upsample(dim * 4, dim * 2)
        self.dec1 = ConvBlock(dim * 3, dim * 2)
        # 在浅层特征空间引入 PA-DMoE，修复精细的空间变异细节
        self.pa_dmoe1 = PA_DSE_AdapterLayer_Ablation(dim * 2, phys_dim, num_experts=num_experts, k=topk)
...
if __name__ == "__main__":
    ...
    model = MoCEIR_PA_DSE_Ablation(inp_channels=in_channels, out_channels=in_channels, dim=feature_dim, phys_dim=physical_embedding_dim, num_experts=num_experts)
