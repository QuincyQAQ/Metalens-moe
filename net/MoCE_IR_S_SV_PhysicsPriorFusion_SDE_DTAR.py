...
########################################################################### 
## Adapter Layer (支持物理先验融合和SDE-DTAR)
class AdapterLayer(nn.Module):
    def __init__(self, 
                 ...
                 use_sde_dtar: bool=True, num_frequency_bands: int=4):
        super().__init__()            
        ...
        self.use_sde_dtar = use_sde_dtar
        self.num_frequency_bands = num_frequency_bands
        
        # SDE-DTAR相关模块
        if use_sde_dtar:
            # 谱域分解模块
            self.sdm = SpectralDecompositionModule(in_channels=dim, num_frequency_bands=num_frequency_bands)
            
            # 计算每个频率子带的通道数
            channels_per_band = (dim * 2) // num_frequency_bands
            
            # 谱域解耦专家
            self.spectral_experts = nn.ModuleList([
                SpectralExpert(channels_per_band, channels_per_band, hidden_dim=32)
        ...
        # 路由函数：如果使用SDE-DTAR，路由针对频率子带；否则针对传统专家
        routing_num_experts = num_frequency_bands if use_sde_dtar else num_experts
        self.routing = DynamicTopologyAwareRouter(
            dim, freq_dim, 
            num_experts=routing_num_experts, k=top_k,
            complexity=expert_complexity, use_complexity_bias=with_complexity, complexity_scale=complexity_scale,
            use_physics_prior=use_physics_prior, num_frequency_bands=num_frequency_bands
        )
    ...
    def forward(self, x, freq_emb, shared, raw_physics_priors=None):
        ...
        if self.use_sde_dtar:
            # SDE-DTAR模式：谱域解耦专家与动态拓扑感知路由
            # 1. 谱域分解
            frequency_bands, padding_info = self.sdm(x)  # List of (B, channels_per_band, H, W//2+1)
            ...
            for i in range(self.num_frequency_bands):
                ...
                # 根据路由权重加权
                routing_weight = gates[:, i].view(B, 1, 1, 1)
                weighted_expert_output_band = expert_output_band * routing_weight
                processed_frequency_bands.append(weighted_expert_output_band)
            
            # 4. 谱域聚合与逆变换
            out = self.sdm.inverse(processed_frequency_bands, original_shape, padding_info)
...
##########################################################################
## 创新点3: 谱域解耦专家与动态拓扑感知路由 (SDE-DTAR)
## 谱域分解模块 (Spectral Decomposition Module, SDM)
class SpectralDecompositionModule(nn.Module):
    """
    谱域分解模块：将输入特征分解到不同的频率子带
    """
    def __init__(self, in_channels, num_frequency_bands=4):
        super().__init__()
        self.num_frequency_bands = num_frequency_bands
    ...
    def forward(self, x):
        ...
        Args:
            x: (B, C, H, W) 输入特征
        Returns:
            frequency_bands: List of (B, C_band, H, W_half) 频率子带特征
            padding_info: (pad_H, pad_W, original_H, original_W) 填充信息和原始尺寸
        """
        B, C, H, W = x.shape
        original_H, original_W = H, W
        
        # 检查是否使用half precision（float16）
        is_half_precision = x.dtype == torch.float16
        ...
        # 如果使用half precision，需要确保维度是2的幂（cuFFT要求）
        pow2_pad_H, pow2_pad_W = 0, 0
        if is_half_precision:
            H_pow2 = self._next_power_of_2(H)
            W_pow2 = self._next_power_of_2(W)
            pow2_pad_H = H_pow2 - H
            pow2_pad_W = W_pow2 - W
            if pow2_pad_H > 0 or pow2_pad_W > 0:
                x = F.pad(x, (0, pow2_pad_W, 0, pow2_pad_H), mode='reflect')
                H, W = x.shape[-2:]
        
        # 应用2D FFT (rfft2用于实数输入，更高效)
        fft_output = torch.fft.rfft2(x, dim=(-2, -1), norm='ortho')  # (B, C, H, W//2+1)
        
        # 将复数分解为实部和虚部
        real_part = fft_output.real  # (B, C, H, W//2+1)
        imag_part = fft_output.imag  # (B, C, H, W//2+1)
        
        # 将实部和虚部拼接，然后按通道分组为频率子带
        fft_combined = torch.cat([real_part, imag_part], dim=1)  # (B, C*2, H, W//2+1)
        
        # 确保通道数可以被频率子带数整除
        total_channels = C * 2
        channels_per_band = total_channels // self.num_frequency_bands
        
        frequency_bands = []
        for i in range(self.num_frequency_bands):
            start_idx = i * channels_per_band
            end_idx = (i + 1) * channels_per_band
            band_features = fft_combined[:, start_idx:end_idx, :, :]
            frequency_bands.append(band_features)
        
        return frequency_bands, (pad_H, pad_W, original_H, original_W, pow2_pad_H, pow2_pad_W)
    
    def inverse(self, frequency_bands_processed, original_shape, padding_info):
        """
        逆变换：将处理后的频率子带聚合并逆FFT回空域
        Args:
            frequency_bands_processed: List of (B, channels_per_band, H, W//2+1) 处理后的频率子带
            original_shape: 原始输入形状 (B, C, H, W)
            padding_info: (pad_H, pad_W, original_H, original_W, pow2_pad_H, pow2_pad_W) 填充信息
        Returns:
            reconstructed_image: (B, C, H, W) 重建的图像
        """
        B, _, H, W_half = frequency_bands_processed[0].shape
        
        # 解析填充信息
        if len(padding_info) == 6:
            pad_H, pad_W, original_H, original_W, pow2_pad_H, pow2_pad_W = padding_info
        else:
            # 向后兼容旧格式
            pad_H, pad_W = padding_info[:2]
            original_H, original_W = original_shape[-2], original_shape[-1]
            pow2_pad_H, pow2_pad_W = 0, 0
        
        # 拼接所有频率子带
        combined_fft_features = torch.cat(frequency_bands_processed, dim=1)  # (B, C*2, H, W//2+1)
        
        # 分离实部和虚部
        C_original = original_shape[1]
        ...
        # 计算填充后的尺寸（H和W_half已经包含了所有填充）
        # W_half = (W_padded // 2) + 1，所以 W_padded = (W_half - 1) * 2
        W_padded = (W_half - 1) * 2 if W_half > 1 else 1
        H_padded = H
        
        # 逆FFT：使用填充后的尺寸
        reconstructed_image = torch.fft.irfft2(
            reconstructed_fft_output, 
            s=(H_padded, W_padded), 
            dim=(-2, -1), 
            norm='ortho'
        )
        ...
        # 移除power-of-2填充（如果存在）
        if pow2_pad_H > 0 or pow2_pad_W > 0:
            reconstructed_image = reconstructed_image[:, :, :H_padded - pow2_pad_H, :W_padded - pow2_pad_W]
            H_padded -= pow2_pad_H
            W_padded -= pow2_pad_W
        
        # 移除奇偶填充（如果存在）
        if pad_H or pad_W:
            reconstructed_image = reconstructed_image[:, :, :original_H, :original_W]
        
        return reconstructed_image


## 谱域解耦专家 (Spectral-Decoupled Expert, SDE)
class SpectralExpert(nn.Module):
    """
    谱域解耦专家：专门处理特定频率子带的轻量级网络
    """
    def __init__(self, in_channels_band, out_channels_band, hidden_dim=32):
        super().__init__()
        # 轻量级网络，专门用于特定频率子带的修复
        self.net = nn.Sequential(
            nn.Conv2d(in_channels_band, hidden_dim, kernel_size=3, padding=1, bias=False),
            nn.GELU(),
            nn.Conv2d(hidden_dim, hidden_dim, kernel_size=3, padding=1, bias=False),
            nn.GELU(),
            nn.Conv2d(hidden_dim, out_channels_band, kernel_size=3, padding=1, bias=False)
        )
    
    def forward(self, x):
        """
        Args:
            x: (B, in_channels_band, H, W//2+1) 频率子带特征
        Returns:
            out: (B, out_channels_band, H, W//2+1) 修复后的频率子带
        """
        return self.net(x)
...
##########################################################################
## 创新点2: 自适应物理约束路由
## 创新点3: 动态拓扑感知路由器 (Dynamic Topology-Aware Router, DTAR)
class DynamicTopologyAwareRouter(nn.Module):
    """
    动态拓扑感知路由器：利用GNN或注意力机制建模频率分量间的依赖关系
    """
    def __init__(self, dim, freq_dim, num_experts, k, complexity, use_complexity_bias: bool = True, complexity_scale: str="max",
                 use_physics_prior: bool=False, num_frequency_bands: int=4, H=None, W=None):
        super(DynamicTopologyAwareRouter, self).__init__()
        ...
        self.gate = nn.Sequential(
            ...
            nn.Linear(dim, num_frequency_bands, bias=False)  # 输出频率子带的路由权重
        ) 
        self.freq_gate = nn.Linear(freq_dim, num_frequency_bands, bias=False)
        
        # 动态拓扑感知：使用自注意力机制建模频率子带间的依赖关系
        self.topology_attention = nn.MultiheadAttention(
            embed_dim=dim, 
            num_heads=8, 
            batch_first=True,
            dropout=0.1
        ...
        # 频率子带特征投影（用于构建拓扑图）
        self.freq_band_projections = nn.ModuleList([
            nn.Linear(dim, dim) for _ in range(num_frequency_bands)
        ])
    ...
    def forward(self, x, freq_emb, raw_physics_priors=None):
        ...
        # 构建频率拓扑图：将输入特征投影为频率子带表示
        x_pooled = F.adaptive_avg_pool2d(x, (1, 1)).squeeze(-1).squeeze(-1)  # (B, C)
        
        # 为每个频率子带创建节点特征
        freq_band_nodes = []
        for proj in self.freq_band_projections:
            node_feat = proj(x_pooled).unsqueeze(1)  # (B, 1, C)
            freq_band_nodes.append(node_feat)
        
        freq_band_nodes = torch.cat(freq_band_nodes, dim=1)  # (B, num_frequency_bands, C)
        
        # 使用自注意力机制建模频率子带间的拓扑关系
        topology_aware_features, _ = self.topology_attention(
            freq_band_nodes, freq_band_nodes, freq_band_nodes
        )  # (B, num_frequency_bands, C)
        
        # 聚合拓扑感知特征
        topology_pooled = topology_aware_features.mean(dim=1)  # (B, C)
        ...
        if self.use_physics_prior:
            ...
            # 融合所有特征用于路由选择
            fused_feat_for_selector_list = [topology_pooled]
            if len(encoded_physics_priors_pooled) > 0:
    ...
    def load_loss(self, logits, logits_noisy, noise_std):
        ...
        threshold_per_item = torch.sum(
            F.one_hot(thresholds, self.num_frequency_bands) * logits_noisy,
            dim=-1
        )
...
# 保持向后兼容性：RoutingFunction作为DynamicTopologyAwareRouter的别名
RoutingFunction = DynamicTopologyAwareRouter
...
class DecoderBlock(nn.Module):
    def __init__(self, dim, num_heads, ffn_expansion_factor, bias, LayerNorm_type, expert_layer, complexity_scale=None,
        ...
        self.adapter = AdapterLayer(
            ...
            use_sde_dtar=use_sde_dtar, num_frequency_bands=num_frequency_bands
        )
...
def build_model(opt):
    ...
    use_sde_dtar = getattr(opt, 'use_sde_dtar', True)
    num_frequency_bands = getattr(opt, 'num_frequency_bands', 4)
