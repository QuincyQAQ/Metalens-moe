...
def train_options():
    ...
    # 合并所有配置到一个命名空间对象
    # 支持多个数据集路径（DATA_FILE_DIRS）或单个数据集路径（DATA_FILE_DIR）
    # 如果设置了环境变量 MOCEIR_TRAIN_DATA_FILE_DIR，则只使用该数据集（用于循环训练多个数据集）
    env_data_dir = os.environ.get("MOCEIR_TRAIN_DATA_FILE_DIR")
    if env_data_dir:
        # 使用环境变量指定的单个数据集
        data_dir_path = Path(str(env_data_dir)).expanduser()
        if not data_dir_path.is_absolute():
            data_dir_path = (project_dir / data_dir_path).resolve()
        else:
            data_dir_path = data_dir_path.resolve()
        data_file_dir = str(data_dir_path)
        data_file_dirs = [data_file_dir]
    elif hasattr(config, "DATA_FILE_DIRS") and isinstance(config.DATA_FILE_DIRS, list):
        # 处理多个数据集路径
        data_file_dirs = []
        for data_dir in config.DATA_FILE_DIRS:
            data_dir_path = Path(str(data_dir)).expanduser()
            if not data_dir_path.is_absolute():
                data_dir_path = (project_dir / data_dir_path).resolve()
            else:
                data_dir_path = data_dir_path.resolve()
            data_file_dirs.append(str(data_dir_path))
        # 为了兼容性，将第一个数据集路径作为data_file_dir
        data_file_dir = data_file_dirs[0] if data_file_dirs else ""
    else:
        # 兼容旧的单个数据集路径配置
        data_file_dir = Path(str(getattr(config, "DATA_FILE_DIR", ""))).expanduser()
    ...
    for data_dir in data_file_dirs:
        if not os.path.exists(data_dir):
            alt = None
            if "/media/" in data_dir:
                if "/data/" in data_dir:
                    alt = data_dir.replace("/data/", "/Data/")
                elif "/Data/" in data_dir:
                    alt = data_dir.replace("/Data/", "/data/")
            if alt and os.path.exists(alt):
                print(f"[Warn] data_file_dir 不存在：{data_dir}，自动改用：{alt}")
                corrected_dirs.append(alt)
            else:
                corrected_dirs.append(data_dir)
        else:
            corrected_dirs.append(data_dir)
    data_file_dirs = corrected_dirs
    data_file_dir = data_file_dirs[0] if data_file_dirs else ""
    ...
    base_kwargs = dict(
        ...
        data_file_dirs=data_file_dirs,  # 多个数据集路径
