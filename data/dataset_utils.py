...
class AIOTrainDataset(Dataset):
    ...
    def _init_synllie(self, id):
        # 获取所有数据集路径
        data_file_dirs = getattr(self.args, "data_file_dirs", None)
        if data_file_dirs and isinstance(data_file_dirs, list) and len(data_file_dirs) > 0:
            data_dirs = data_file_dirs
        else:
            data_dirs = [self.args.data_file_dir]
        
        self.synllie_lr = []
        self.synllie_hr = []
        
        for data_file_dir in data_dirs:
            inputs = data_file_dir + "/llie/LOLv1/Train/input"
            targets = data_file_dir + "/llie/LOLv1/Train/target"
            if os.path.exists(inputs) and os.path.exists(targets):
                lr_list = [{"img" : x, "de_type":id} for x in sorted(glob.glob(inputs + "/*.png"))]
                hr_list = [{"img" : x, "de_type":id} for x in sorted(glob.glob(targets + "/*.png"))]
                self.synllie_lr.extend(lr_list)
                self.synllie_hr.extend(hr_list)
        
        self.synllie_counter = 0
        if _is_main_process():
            print("SynLLIE training pairs: {} (from {} dataset(s))".format(len(self.synllie_lr), len(data_dirs)))
    ...
    def _init_deblur(self, id):
        ...
        支持多个数据集路径：如果args中有data_file_dirs列表，会从所有路径加载并合并数据
        """
        # 获取所有数据集路径
        data_file_dirs = getattr(self.args, "data_file_dirs", None)
        if data_file_dirs and isinstance(data_file_dirs, list) and len(data_file_dirs) > 0:
        ...
        for data_file_dir in data_dirs:
            # 1) Metalens_0622
            metalens_inputs = os.path.join(data_file_dir, "Metalens_0622", "train", "meta")
            metalens_targets = os.path.join(data_file_dir, "Metalens_0622", "train", "ground_truth")

            # 2) 通用 train/lr, train/gt 结构（例如 open_dataset_8_1_1）
            generic_inputs = os.path.join(data_file_dir, "train", "lr")
        ...
        if _is_main_process():
            print("Deblur training pairs: {} (from {} dataset(s))".format(len(self.deblur_hr), len(data_dirs)))
    ...
    def _init_derain(self, id):
        ...
        for data_file_dir in data_dirs:
            inputs = data_file_dir + "/deraining/RainTrainL/rainy"
            targets = data_file_dir + "/deraining/RainTrainL/gt"
            if os.path.exists(inputs) and os.path.exists(targets):
                lr_list = [{"img" : x, "de_type":id} for x in sorted(glob.glob(inputs + "/*.png"))]
                hr_list = [{"img" : x, "de_type":id} for x in sorted(glob.glob(targets + "/*.png"))]
                self.derain_lr.extend(lr_list)
                self.derain_hr.extend(hr_list)
        
        self.derain_counter = 0
        if _is_main_process():
            print("Derain training pairs: {} (from {} dataset(s))".format(len(self.derain_lr), len(data_dirs)))
    ...
    def _init_dehaze(self, id):
        ...
        for data_file_dir in data_dirs:
            inputs = data_file_dir + "/dehazing/RESIDE/"
            targets = data_file_dir + "/dehazing/RESIDE/clear"
            if os.path.exists(inputs) and os.path.exists(targets):
                for part in ["part1", "part2", "part3", "part4"]:
                    part_path = inputs + part
                    if os.path.exists(part_path):
                        self.dehaze_lr.extend([{"img" : x, "de_type":id} for x in sorted(glob.glob(part_path + "/*.jpg"))])
                if os.path.exists(targets):
                    self.dehaze_hr.extend([{"img" : x, "de_type":id} for x in sorted(glob.glob(targets + "/*.jpg"))])
        
        self.dehaze_counter = 0
        if _is_main_process():
            print("Dehaze training pairs: {} (from {} dataset(s))".format(len(self.dehaze_lr), len(data_dirs)))
    ...
    def _init_clean(self, id):
        ...
        clean = []
        for data_file_dir in data_dirs:
            inputs = data_file_dir + "/denoising"
            if os.path.exists(inputs):
                for dataset in ["WaterlooED", "BSD400"]:
                    if dataset == "WaterlooED":
                        ...
                        ext = "jpg"
                    dataset_path = inputs + f"/{dataset}"
                    if os.path.exists(dataset_path):
                        clean.extend([x for x in sorted(glob.glob(dataset_path + f"/*.{ext}"))])
        ...
        if _is_main_process():
            print("Denoise training pairs: {} (from {} dataset(s))".format(self.num_clean, len(data_dirs)))
